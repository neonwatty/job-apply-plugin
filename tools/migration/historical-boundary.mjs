...
export function historicalRecord(path) {
  ...
  // Current requirement declarations map tests but never grant acceptance. The
  // original requirements and all archived task evidence remain immutable.
  return !/^config\/migration\/(?:source-catalog-[^/]+\.json|[^/]+-surfaces\.json|requirements-current-[^/]+\.json|python-runtime-closure\.json|review-lock\.json)$/.test(path);
}
