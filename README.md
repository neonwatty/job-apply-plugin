...
`prepare` prints a unique route fragment and creates a unique run ID. The host resolves that route to the isolated store with `python3 scripts/qa-replay.py resolve --route-token 'GENERATED_RUN_ID.GENERATED_ROUTE_TOKEN'`. After evaluation—or to abandon an interrupted run—remove private run artifacts with `python3 scripts/qa-replay.py cleanup --run-id GENERATED_RUN_ID`. The dated session, fixture, and run IDs above are examples: testers must replace them with unique values for every recording, fixture revision, and replay. Current replay run IDs are generated by `prepare`; use the generated ID in place of the illustrative `qa-run-20260811-001` value.
