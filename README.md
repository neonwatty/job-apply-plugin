...
`prepare` prints a unique route fragment and creates a unique run ID. The host resolves that route to the isolated store with `python3 scripts/qa-replay.py resolve --route-token 'GENERATED_RUN_ID.GENERATED_ROUTE_TOKEN'`. After evaluation—or to abandon an interrupted run—sanitize the run with `python3 scripts/qa-replay.py cleanup --run-id GENERATED_RUN_ID`. Cleanup leaves a minimal tombstone; completed runs retain only the redacted report, while abandoned runs retain no report. The dated session, fixture, and run IDs above are examples: testers must replace them with unique values for every recording, fixture revision, and replay. Current replay run IDs are generated by `prepare`; use the generated ID in place of the illustrative `qa-run-20260811-001` value.

The committed `linkedin-screening` lane is wholly synthetic and stops at review. Its top-choice and sponsorship answers remain browser-local, and the final submit action must never be clicked: a final-action activation fails the oracle. The coordinator does not launch an agent or submit an application. Live LinkedIn, authentication, real applicant material, and external ATS capture are outside this deterministic lane. The earlier `complete-profile` scenario remains supported for its matching short fixture.
