...
function revision(value, label = 'expectedRevision', zero = false) {
...
export async function automationHttp(repository, method, path, body) {
    ...
    if (method === 'GET' && path === '/api/automation') {
        ...
        return response(set(projection, 'applicationAuthority', authority ? await authority.status() : object(parse('{"mode":"guided","status":"active","revision":0,"authorizationId":null,"expiresAt":null,"runId":null,"jobIds":[],"sensitiveAnswerRefs":[]}'), 'application authority projection')));
    }
    if (method === 'GET' && path === '/api/application-authority/progress')
        return authority ? response(await authority.progress()) : null;
    ...
    if (control) {
        exact(payload, ['expectedRevision'], 'body must contain expectedRevision');
        return response(await authority.control(control[1], revision(get(payload, 'expectedRevision'))));
    }
    if (path === '/api/application-authority') {
        ...
        if (!(get(payload, 'authority') instanceof PythonObject))
            throw new JobsError('body must contain authority and expectedRevision');
