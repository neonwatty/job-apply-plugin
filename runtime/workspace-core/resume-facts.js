...
function versions(document, resumeId) {
    const value = get(records(document, 'sets'), resumeId);
    return value === null ? null : get(object(value, 'resume fact set'), 'versions');
}
function latest(document, resumeId) {
    const entries = versions(document, resumeId);
    return entries?.length ? object(entries[entries.length - 1], 'resume fact version') : null;
}
function summary(record, resumeId, currentContentRevision) {
...
function appendDraft(tx, resumeId, contentRevision, candidate, expectedFactRevision, now) {
...
export class ResumeFactsService extends ExtractionContext {
    ...
    createDraft(resumeId, candidate, expectedResumeRevision, expectedFactRevision) {
        safeId(resumeId);
    ...
    completeRequest(id, candidate, expectedRequest) {
        safeId(id);
        validatedCandidate(candidate);
        return this.repository.extractionTransaction(async (tx) => {
            ...
            if (content !== string(get(request, 'resumeContentRevision')))
    ...
    confirm(resumeId, expectedFactRevision, expectedContentRevision) {
        safeId(resumeId);
        return this.repository.extractionTransaction(async (tx) => {
            const resume = await readyResume(tx, resumeId);
