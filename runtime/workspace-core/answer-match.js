...
export function semanticCandidate(record) {
...
export function semanticLookup(document, incoming) {
    ...
    if (keys(packet).some(key => !allowed.includes(key)) || required.some(key => !packet.has(text(key))))
        throw new JobsError('semantic lookup contains unsupported fields');
    ...
    try {
        ...
        throw new JobsError('semantic lookup is invalid');
    }
}
