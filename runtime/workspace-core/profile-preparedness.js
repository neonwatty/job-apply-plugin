...
function meaningful(value) {
    ...
    if (value instanceof PythonObject)
        return value.entries().some(([, item]) => meaningful(item));
    return truth(value);
}
function userProtects(provenance, path) {
    for (const [key, value] of provenance.entries()) {
        const pointer = string(key);
        if ((path === pointer || path.startsWith(`${pointer}/`)) && string(get(object(value, 'provenance'), 'source')) === 'user')
            return true;
    }
    return false;
}
export class PreparednessService {
    ...
    async get() {
        ...
        const resumeItem = { id: 'default_resume', state: 'blocked', reasonCode: 'default_resume_missing' };
        if (resume) {
            resumeItem.resumeId = resume.id;
            if (resume.storageKind !== 'managed')
                resumeItem.reasonCode = 'default_resume_unreadable';
            else {
                const observed = await snapshot.observeResume(resume);
                if (!observed.exists)
                    resumeItem.reasonCode = 'default_resume_unreadable';
                else if (observed.digest !== resume.digest)
                    resumeItem.reasonCode = 'default_resume_changed';
                else {
                    resumeItem.state = 'present';
                    resumeItem.reasonCode = null;
                }
            }
        }
        essentialSetup.push(resumeItem);
        const groups = [
        ...
        for (const request of snapshot.requests)
            if (['requested', 'failed', 'stale'].includes(request.status)) {
                const item = { kind: 'extraction_request', reasonCode: `extraction_${request.status}`,
        ...
        for (const proposal of snapshot.proposals)
            if (proposal.status === 'pending' && proposal.pendingPaths.length) {
                ...
                if (count)
                    reviewHealth.push({ kind: 'resume_proposal', reasonCode: 'human_protected_facts_retained',
