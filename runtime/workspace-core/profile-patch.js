...
export function topKey(path) {
    if (!path.startsWith('/') || path.slice(1).includes('/'))
        throw new JobsError('atomic profile paths must identify one top-level fact');
    if (path.length === 1 || /~(?![01])/.test(path))
        throw new JobsError('atomic profile path is invalid');
    return path.slice(1).replaceAll('~1', '/').replaceAll('~0', '~');
}
function merge(target, patch, prefix = '') {
    const updated = copy(target), changed = [];
    for (const [key, value] of patch.entries()) {
        const name = string(key);
        if (!name)
            throw new JobsError('profile patch keys must be non-empty strings');
...
export function applyPatch(target, patch, atomic, deleted) {
    const atomicKeys = new Map(atomic.map(path => [topKey(path), path]));
    if (atomicKeys.size !== atomic.length || new Set(deleted).size !== deleted.length)
        throw new JobsError('atomic profile paths must be unique');
    if (deleted.some(path => !atomic.includes(path)))
        throw new JobsError('deleted profile paths must also be atomic');
    if ([...atomicKeys.keys()].some(key => !has(patch, key)))
        throw new JobsError('atomic profile paths must be present in the patch');
    const ordinary = copy(patch);
    for (const key of atomicKeys.keys())
        ordinary.delete(text(key));
    const [updated, changed] = merge(target, ordinary);
    for (const [key, path] of atomicKeys) {
        if (deleted.includes(path)) {
            if (updated.delete(text(key)))
                changed.push(path);
        }
...
export function changedPaths(current, next, prefix = '') {
...
export function provenanceAfter(provenance, changed, source, now, current) {
    ...
    for (const [key, record] of provenance.entries()) {
        ...
        if (changed.some(item => item.startsWith(`${path}/`))) {
            for (const leaf of leaves(at(current, path), path))
                if (!has(stamped, leaf))
                    set(stamped, leaf, record);
        }
    }
    for (const path of changed) {
        for (const key of keys(stamped))
            if (key.startsWith(`${path}/`) || path.startsWith(`${key}/`))
                stamped.delete(text(key));
