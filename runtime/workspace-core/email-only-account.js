...
async function executeEmailOnly(repository, request, executor, nowValue, authorize, exactPortal = false) {
    return repository.accountOperationTransaction(async (tx) => {
        ...
        if (authorize)
            request = await authorize(operation, now);
...
export class LiveEmailOnlyAccountService {
    ...
    authority;
    now;
    constructor(repository, executor, authority, now = () => new Date().toISOString().replace(/\.\d{3}Z$/, 'Z')) {
        this.repository = repository;
        this.executor = executor;
        this.authority = authority;
        this.now = now;
        if (executor.providerId !== 'macos-accessibility')
            throw new JobsError('live account canary native boundary is unavailable');
    }
    execute(value) {
