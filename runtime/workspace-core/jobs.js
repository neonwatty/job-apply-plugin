...
export class JobsService {
    ...
    now;
    id;
    constructor(repository, now = () => new Date().toISOString().replace(/\.\d{3}Z$/, "Z"), id = () => `job-${randomUUID()}`) {
        this.repository = repository;
        this.now = now;
        this.id = id;
    }
    async create(value, author = "human") {
        ...
        if (has(input, "status") && string(get(input, "status")) !== "saved")
            throw new JobsError("new jobs must start with saved status");
        ...
        return this.repository.transaction(async (transaction) => {
            ...
            if (has(jobs, id))
                throw new JobsError("job id already exists");
    ...
    async get(id, includeTrashed = false) {
        safeId(id);
    ...
    async update(id, value, expectedRevision, author = "human") {
        ...
        return this.repository.transaction(async (transaction) => {
            ...
            for (const field of keys(patch)) {
                ...
                if (field === "provenance" || by === "agent" && (!nonempty(value) || !mayUpdate(current, currentProvenance, field)))
                    continue;
                set(updated, field, value);
                accepted.push(field);
            }
            if (accepted.includes("resumeId"))
                await transaction.requireResume(get(updated, "resumeId"));
            ...
            if (!changed.length && same(provenance, currentProvenance))
                return current;
            const now = this.now();
    ...
    requireUnique(jobs, normalized, except) {
