...
export async function runSessionTransaction(storage, operation) {
...
export function runReplayTransitionTransaction(storage, operation) {
    ...
    return runSessionTransaction(storage, session => {
        const requireStandalone = async (id) => {
...
export async function preparednessSnapshot(storage) {
    ...
    const resumeRecords = new Map();
    const resumes = object(get(resumesDocument, 'resumes'), 'resumes.resumes').entries().map(([key, value]) => {
        const record = object(value, 'resume record'), id = safeId(string(key));
        resumeRecords.set(id, record);
    ...
    const proposals = object(get(proposalsDocument, 'proposals'), 'resume extraction proposals').entries().map(([, value]) => {
        ...
        return { status: string(get(record, 'status')), resumeId: string(get(record, 'resumeId')), id: string(get(record, 'id')),
            pendingPaths: get(record, 'pendingPaths').map(item => string(item)) };
    });
...
export function stateStorage(root, read, document, write = atomicWritePointJson) {
