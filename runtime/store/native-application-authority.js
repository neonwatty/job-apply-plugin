...
const exact = (value, fields) => value.size === fields.length && keys(value).every(key => fields.includes(key));
...
function guidedFromLegacyAuthority(value, now) {
    if (!exact(value, ['schemaVersion', 'activeAuthorityId', 'authorities', 'metadata']) || int(get(value, 'schemaVersion')) !== 1n)
        return null;
    try {
        ...
        for (const [key, raw] of authorities.entries()) {
            ...
            if (!exact(record, ['authorizationId', 'mode', 'status', 'revision', 'issuedAt', 'expiresAt', 'revokedAt',
                ...
                || !stringList(get(record, 'workerIds')) || !stringList(get(record, 'sensitiveFieldClasses')))
                return null;
        ...
        if (get(value, 'activeAuthorityId') !== null && (!active || get(authorities, active) === null
            || string(get(object(get(authorities, active), 'legacy active authority'), 'status')) !== 'active'))
            return null;
        ...
        if (!exact(metadata, ['revision', 'createdAt', 'updatedAt']) || revision === null || revision < 0n
            || !string(get(metadata, 'createdAt')) || !string(get(metadata, 'updatedAt')))
            return null;
        const guided = initialApplicationAuthorityDocument(now), next = object(get(guided, 'metadata'), 'authority metadata');
        set(next, 'revision', get(metadata, 'revision'));
        set(next, 'createdAt', get(metadata, 'createdAt'));
        set(next, 'updatedAt', get(metadata, 'updatedAt'));
        return guided;
    }
    catch {
        return null;
...
export async function loadApplicationAuthority(read, now) {
    try {
        const value = await read('application-authority');
        try {
            return validateApplicationAuthorityDocument(value);
        }
        catch (error) {
...
export async function applicationAuthorityTransaction(access, operation) {
