...
export function resumeModifiedAt(mtimeSeconds) {
    if (Number.isNaN(mtimeSeconds))
        throw new ResumeTimestampError("ValueError", "Invalid value NaN");
    if (!Number.isFinite(mtimeSeconds))
        throw new ResumeTimestampError("OverflowError", "timestamp is infinite");
    ...
    if (year < 1n || year > 9999n)
        throw new ResumeTimestampError("ValueError", "year must be in 1..9999");
