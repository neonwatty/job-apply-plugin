...
function failure(message = 'native policy tree is unsupported') {
    return new JobsError(message);
}
function ownedPrivate(metadata) {
    return metadata.uid === BigInt(process.getuid()) && (metadata.mode & 63n) === 0n;
}
function ownedStructural(metadata) {
    return metadata.uid === BigInt(process.getuid()) && (metadata.mode & 18n) === 0n;
}
async function privateDirectory(path, structural = false) {
...
async function unchangedDirectory(path, before, structural = false) {
    ...
    if (!after?.isDirectory() || after.isSymbolicLink() || after.dev !== before.dev || after.ino !== before.ino
        ...
        || Number(after.mode & 511n) !== before.mode
        || !(structural ? ownedStructural(after) : ownedPrivate(after)))
        throw failure('native policy tree changed');
}
function validateFile(metadata) {
    if (!metadata.isFile() || metadata.isSymbolicLink() || metadata.nlink !== 1n || !ownedPrivate(metadata)
        || metadata.size > BigInt(maximumFileBytes))
        throw failure();
}
async function privateBytes(path) {
    ...
    try {
        ...
        if (!linked || linked.dev !== before.dev || linked.ino !== before.ino)
            throw failure('native policy tree changed');
        ...
        if (!final || before.dev !== after.dev || before.ino !== after.ino || before.size !== after.size
            ...
            || final.dev !== after.dev || final.ino !== after.ino)
            throw failure('native policy tree changed');
...
async function acquirePolicyLock(path, provider, signal) {
    let handle;
    try {
        handle = await open(path, constants.O_RDWR | constants.O_NOFOLLOW | constants.O_NONBLOCK);
    }
    catch {
        throw failure('native policy tree is unsupported: lock is unavailable');
    }
    try {
        ...
        while (true) {
            ...
            if (provider.tryLock(handle.fd))
                break;
            await delay(10, undefined, { signal });
        }
        const linked = await lstat(path, { bigint: true }).catch(() => null);
        const locked = await handle.stat({ bigint: true });
        if (!linked || linked.dev !== locked.dev || linked.ino !== locked.ino)
            throw failure('native policy lock changed');
        return handle;
    }
    catch (error) {
        await handle.close();
...
async function snapshot(policyRoot) {
    const rootIdentity = await privateDirectory(policyRoot);
    const entries = [{ relative: policyName, type: 'directory', mode: rootIdentity.mode }];
    let aggregate = 0;
    const addFile = async (relative) => {
        ...
        if (entries.length > maximumEntries || aggregate > maximumTreeBytes)
            throw failure('native policy tree is unbounded');
    };
    const addDirectory = async (relative) => {
    ...
    for (const name of rootNames) {
        ...
        if (name === 'campaigns') {
            ...
            for (const campaign of campaigns) {
                ...
                if (applications.some(child => !hexadecimalName.test(child)))
                    throw failure();
                for (const application of applications)
                    await addFile(`${campaignRelative}/${application}`);
...
export async function withNativePolicyTree(storeRoot, provider, operation, signal = AbortSignal.timeout(30_000)) {
    const policyRoot = join(storeRoot, policyName);
    let metadata;
    try {
        metadata = await lstat(policyRoot);
    }
    catch (error) {
    ...
    try {
        ...
        try {
            ...
            if (error instanceof Error && failureValue !== undefined && error.cause === undefined)
                error.cause = failureValue;
            throw error;
        }
    }
}
export function updateNativePolicyDigest(digest, entries) {
    if (!entries)
        return;
    for (const entry of entries) {
        ...
        if (entry.type === 'file')
            digest.update(`:${entry.bytes.length}:`).update(entry.bytes);
        else
            digest.update(':');
    }
}
...
export async function copyNativePolicyTree(targetRoot, entries) {
    if (!entries)
        return;
