...
export function validateResolutionJournal(journal) {
    ...
    if (operation.size !== fields.length || keys(operation).some(key => !fields.includes(key)) || string(get(operation, 'kind')) !== 'answer_resolution' || get(operation, 'resultClaim') !== null)
        throw new JobsError('coordinator answer resolution operation is invalid');
    const id = safeId(string(get(operation, 'jobId')));
    for (const field of ['operationId', 'at', 'answerKey'])
    ...
    for (const field of ['expectedJobRevision', 'expectedSessionRevision', 'expectedAnswerRevision']) {
        ...
        if (value === null || value < 1n)
            throw new JobsError('coordinator answer resolution revision is invalid');
    }
    if (string(get(operation, 'sourceStatus')) !== 'needs_info' || !['needs_info', 'ready'].includes(string(get(operation, 'targetStatus'))))
        throw new JobsError('coordinator answer resolution status is invalid');
    if (string(get(validateAnswerSession(get(operation, 'session')), 'applicationId')) !== id)
        throw new JobsError('coordinator answer resolution session identity is invalid');
    return operation;
}
function projection(journal, jobs, sessions) {
    ...
    else if (int(get(job, 'revision')) !== expected + 1n || string(get(job, 'status')) !== string(get(operation, 'targetStatus')))
    ...
    if (!existing)
        throw new JobsError('coordinator resolution references a missing session');
    ...
    if (!changedSession && canonicalJson(existing) !== canonicalJson(session))
        throw new JobsError('coordinator session cannot be reconciled');
...
export class NativeAnswerResolutionJournal {
    ...
    async recover(journal, jobs, sessions) {
        ...
        if (plan.changedJob)
            await this.write('jobs', plan.next);
        if (plan.changedSession)
            await this.write(`sessions/${plan.id}`, plan.session);
    ...
    async commit(operation, jobs, sessions) {
