...
function sameIdentity(left, right) {
    ...
    for (let index = 0; index < right.length; index += 1) {
        ...
        if (typeof numeric === "bigint") {
            if (numeric !== right[index])
                return false;
        }
        else if (!Number.isInteger(numeric) || BigInt(numeric) !== right[index])
            return false;
    }
    return true;
}
function osError(error) {
...
function existingKey(cache, key) {
    for (const candidate of cache.keys()) {
        if (sameKey(hashable(candidate), key))
            return candidate;
    }
    return key;
}
/** Inert observation flow. Only its explicitly supplied cache may be mutated. */
export async function managedResumeObservation(record, io, digestCache = null) {
    const path = await io.managedPath(record);
    if (!record.has("id"))
        throw new ObservationKeyError("id");
    ...
    if (await io.isSymlink(path) || !metadata.isRegularFile || metadata.size > 10485760n)
        return missing;
    ...
    if (cacheIdentity !== null && cached !== undefined && cached !== null && sameIdentity(cached.identity, cacheIdentity)) {
        if (!("checkedAt" in cached))
            throw new ObservationKeyError("checkedAt");
        const elapsed = now - cached.checkedAt;
        if (elapsed >= 0n && elapsed < 30000000n) {
            const size = metadata.size;
            const modifiedAt = resumeModifiedAt(metadata.mtimeSeconds);
            if (!("digest" in cached))
                throw new ObservationKeyError("digest");
    ...
    if (!sameIdentity(afterIdentity, beforeIdentity) || !after.isRegularFile)
        return missing;
    const observation = {
