...
export async function withExclusiveFileLock(path, callback, options) {
    ...
    try {
        while (true) {
            signal.throwIfAborted();
            if (options.provider.tryLock(handle.fd))
                break;
            try {
        ...
        try {
            ...
            throw contextual(error, failure);
        }
    }
}
