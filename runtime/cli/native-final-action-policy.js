...
async function readInput(path) {
    ...
    try {
        ...
        if (path === '-') {
            ...
            text = decodePolicyBytes(await readFile(expandRoot(path)));
        value = parsePolicyJson(text);
    }
    catch {
        throw new PolicyError('input is not a readable JSON object');
    }
    return object(value, 'input');
}
export async function runPolicyCli(args) {
    ...
    const get = (name) => typeof parsed.values[name] === 'string' ? parsed.values[name] : undefined;
    const required = (name) => {
    ...
    if (command === 'activate')
        return service.activate(await readInput(required('input')));
    if (command === 'authorize')
        return service.authorize(await readInput(required('input')));
    ...
    if (command === 'record-outcome') {
        ...
        return service.outcome(required('campaign-id'), required('application-ref'), required('lease-id'), required('claim-id'), required('outcome'), eventPath ? await readInput(eventPath) : undefined, get('confirmation-capability'));
    }
    if (command === 'kill')
        return service.kill();
