...
export async function runAttemptCli(args) {
    let invalid = false;
    try {
        ...
        let invocation;
        try {
            invocation = protocol.parseAttemptArgs(args, process.env, homedir());
        }
        catch (error) {
            ...
            invalid = error instanceof protocol.AttemptInvocationError;
            throw error;
        }
