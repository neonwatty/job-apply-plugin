...
export const pendingAnswerCommands = {
    'pending-answer-list': [],
...
export async function runPendingAnswerCommand(command, repository, options) {
    const service = new PendingAnswersService(repository);
    if (command === 'pending-answer-list')
        return service.list();
    const required = (key) => {
    ...
    return service.resolve(required('--id'), required('--reference'), revision('--expected-job-revision'), revision('--expected-session-revision'), revision('--expected-answer-revision'), options.has('--owner-confirmed'));
}
