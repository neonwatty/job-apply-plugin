...
export async function runExtractionCommand(command, repository, options, payload) {
    ...
    switch (command) {
        case 'resume-extraction-request-create': {
            ...
            if (scope !== undefined && scope !== 'resume')
        ...
        case 'resume-facts-draft': return facts.createDraft(required('--resume-id'), await payload(), revision('--expected-resume-revision'), options.has('--expected-fact-revision') ? revision('--expected-fact-revision') : null);
