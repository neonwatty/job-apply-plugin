...
export function policyReasons(record) {
    ...
    if (!['active', 'deleted'].includes(status))
        throw new AnswerMatchError('candidate record status is invalid');
    if (!['accepted', 'pending', 'declined'].includes(review))
        throw new AnswerMatchError('candidate review status is invalid');
    if (!['confirmed', 'inferred', 'missing', 'sensitive'].includes(state))
        throw new AnswerMatchError('candidate state is invalid');
    if (!['seen', 'unseen', 'missing'].includes(value))
        throw new AnswerMatchError('candidate value state is invalid');
...
export function evaluateReuse(input) {
    if (!(input.match instanceof PythonObject) || !(input.candidate instanceof PythonObject))
        throw new AnswerMatchError('reuse input is invalid');
    ...
    if (string(get(match, 'answerKey')) !== key || !['exact', 'high', 'uncertain', 'none'].includes(band)
        || !Array.isArray(reasons) || reasons.some(value => !reasonCodes.has(string(value))))
        throw new AnswerMatchError('match result is invalid');
    ...
    if (!['strict', 'bounded_loose'].includes(mode))
        throw new AnswerMatchError('reuse mode is invalid');
    if (!['none', 'accepted_record', 'per_use', 'bounded_policy'].includes(authority))
        throw new AnswerMatchError('use authority is invalid');
    const allowlist = input.allowedSensitiveFieldClasses === undefined ? [] : input.allowedSensitiveFieldClasses;
    if (!Array.isArray(allowlist))
        throw new AnswerMatchError('field class allowlist is invalid');
    ...
    if (authority === 'per_use') {
        ...
        if (allowed.has(observedClass)) {
            ...
            output.push('field_class_not_allowlisted');
    }
    else
        output.push('authority_missing');
