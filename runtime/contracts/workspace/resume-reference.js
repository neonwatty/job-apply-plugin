...
export function validateResumeReferences(resumes) {
    let defaults = 0;
    for (const [key, value] of resumes.entries()) {
        const id = safeId(string(key)), record = object(value, "resume record");
        if (keys(record).some(key => !fields.has(key)))
            throw new JobsError("resume record contains unsupported fields");
        if (string(get(record, "id")) !== id)
            throw new JobsError("resume record id does not match its index");
        if (!strip(string(get(record, "label")) ?? ""))
        ...
        if (storage === null) {
            ...
            if (has(record, "path"))
                throw new JobsError("managed resume record must not contain a source path");
            const file = string(get(record, "managedFile"));
            if (!Object.keys(media).some(extension => file === id + extension))
                throw new JobsError("managed resume file identity is invalid");
            ...
            if (string(get(record, "mediaType")) !== media[posix.extname(file)])
                throw new JobsError("managed resume media type is invalid");
            if (!matches(/^[0-9a-f]{64}$/, get(record, "digest")))
                throw new JobsError("managed resume digest is invalid");
            ...
            if (get(record, "observedSize") === null || !string(get(record, "observedModifiedAt")))
                throw new JobsError("managed resume observation is incomplete");
        }
        else
            throw new JobsError("resume storage kind is unsupported");
        const tags = has(record, "tags") ? get(record, "tags") : [];
        if (!Array.isArray(tags) || tags.some(tag => !strip(string(tag) ?? "")))
            throw new JobsError("resume tags must be non-empty strings");
        if (new Set(tags.map(tag => tag.contentKey())).size !== tags.length)
            throw new JobsError("resume tags must be unique");
        if (typeof get(record, "default") !== "boolean")
            throw new JobsError("resume default must be a boolean");
        if (get(record, "default") === true && get(record, "deletedAt") === null)
            defaults++;
        ...
        if (int(get(record, "revision")) === null || int(get(record, "revision")) < 1n)
            throw new JobsError("resume revision must be a positive integer");
        for (const field of ["observedModifiedAt", "createdAt", "updatedAt", "deletedAt"]) {
            if (get(record, field) !== null && string(get(record, field)) === null)
                throw new JobsError(`resume record.${field} must be a string`);
        }
        if (!string(get(record, "createdAt")))
            throw new JobsError("resume record has no creation timestamp");
        if (!string(get(record, "updatedAt")))
            throw new JobsError("resume record has no update timestamp");
    }
    if (defaults > 1)
        throw new JobsError("resume store has more than one active default");
}
