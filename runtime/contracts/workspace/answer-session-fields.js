...
export function fields(record, allowed, exact = false) {
...
export function optionalStrings(record, names, label) {
...
export function validReasons(value) {
    return Array.isArray(value) && value.every(code => member(code, reasonCodes));
}
export function validatePendingFields(value) {
    ...
    for (const item of value) {
        ...
        if (has(field, 'state'))
            requireCondition(member(get(field, 'state'), answerStates), 'pending field state is unsupported');
        if (has(field, 'sensitive'))
            requireCondition(typeof get(field, 'sensitive') === 'boolean', 'pending field sensitive must be a boolean');
        ...
        if (has(field, 'fieldClass'))
            requireCondition(matches(get(field, 'fieldClass'), /^[a-z][a-z0-9_]{0,63}$/u), 'pending field class is invalid');
        for (const [key, label] of [['scopeFingerprint', 'scope'], ['questionFingerprint', 'question']]) {
            if (has(field, key))
                requireCondition(matches(get(field, key), /^[0-9a-f]{64}$/u), `pending field ${label} fingerprint is invalid`);
        }
        if (has(field, 'matchConfidence'))
            requireCondition(member(get(field, 'matchConfidence'), confidenceBands), 'pending field confidence is invalid');
        if (has(field, 'matchAnswerRevision'))
            requireCondition(positive(get(field, 'matchAnswerRevision')), 'pending field match answer revision is invalid');
        if (has(field, 'matchReasonCodes'))
            requireCondition(validReasons(get(field, 'matchReasonCodes')), 'pending field match reasons are invalid');
    }
}
...
export function validateBlockers(value) {
    ...
    for (const item of value) {
        ...
        if (has(blocker, 'reference'))
            requireCondition(matches(get(blocker, 'reference'), pendingReference), 'session blocker reference is invalid');
