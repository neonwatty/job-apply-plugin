...
function rejectCollisions(answers, redirects, merged, winnerKey, sourceKey) {
    ...
    for (const key of keys(answers)) {
        if (key === winnerKey || key === sourceKey)
            continue;
...
export function applyAnswerMerge(document, operation) {
    ...
    if (!has(document, 'redirects'))
        set(document, 'redirects', emptyObject());
    ...
    if (winnerValue === null)
        throw new JobsError('answer merge winner does not exist');
    const winner = object(winnerValue, 'answer');
    const source = object(sourceValue, 'answer');
    if (answerRevision(winner) !== expectedWinner || answerRevision(source) !== expectedSource)
        throw new JobsError('answer merge revision conflict');
    if (get(winner, 'deletedAt') !== null || get(source, 'deletedAt') !== null)
        throw new JobsError('answer merge records must be active');
    if (string(fallback(winner, 'reviewStatus', text('accepted'))) !== 'accepted')
    ...
    const aliases = [];
    for (const value of [
        ...fallback(winner, 'aliases', []),
        get(source, 'question'),
        ...fallback(source, 'aliases', []),
    ]) {
        const raw = nonblank(value);
        if (raw === null)
            continue;
        const normalized = normalizeAnswerQuestion(raw);
        if (normalized !== winnerQuestion && !aliases.includes(normalized))
    ...
    for (const field of ['observedAt', 'lastObservedAt']) {
        const values = [string(get(winner, field)), string(get(source, field))].filter((value) => value !== null && value !== '');
        ...
        if (values.length === 0)
            merged.delete(text(field));
        else
            set(merged, field, text(field === 'observedAt' ? values[0] : values[values.length - 1]));
    }
    ...
    for (const key of keys(redirects)) {
        ...
        if (string(get(redirect, 'targetKey')) === sourceKey)
            set(redirect, 'targetKey', text(winnerKey));
    }
