...
function aggregateControls(request) {
...
export function validateEmailOnlyAccountRequest(value) {
    ...
    if (string(get(request, 'flowKind')) !== emailFlow)
        throw new JobsError('email-only flow kind is invalid');
    if (!string(get(request, 'jobId')) || !string(get(request, 'expectedClaimId')))
        throw new JobsError('email-only job binding is invalid');
    const descriptor = string(get(request, 'realmDescriptor'));
    const realm = string(get(request, 'realmRef'));
    if (!descriptor?.startsWith('oracle-recruiting:v1:'))
        throw new JobsError('email-only realm descriptor is invalid');
    if (!realm || !realmPattern.test(realm)
        || createHash('sha256').update(descriptor).digest('hex') !== realm)
        throw new JobsError('email-only realm identity mismatch');
    for (const field of ['jobRevision', 'accountRevision', 'settingsRevision']) {
        ...
        if (revision === null || revision < 1n)
            throw new JobsError('email-only revision binding is invalid');
    }
    for (const field of ['accountFormFingerprint', 'emailControlFingerprint', 'termsControlFingerprint',
        ...
        if (!fingerprintPattern.test(string(get(request, field)) ?? ''))
            throw new JobsError('email-only control binding is invalid');
    }
    ...
    let portal;
    try {
        portal = new URL(string(get(request, 'portalUrl')) ?? '');
    }
    catch {
        throw new JobsError('email-only portal is invalid');
    }
...
export function validateEmailOnlyAccountResult(value, providerId) {
