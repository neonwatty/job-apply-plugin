...
export function resolveAccountRealm(input) {
    ...
    if (host === 'my.greenhouse.io') {
        const path = parsed[3];
        if (parsed[4] || path.includes('%') || path.includes('//'))
            return unresolved('mygreenhouse_path_unproven');
...
export function classifyAccountFlow(input) {
    const realm = resolveAccountRealm(input);
    if (realm.status === 'resolved')
        return { ...realm, status: 'classified',
