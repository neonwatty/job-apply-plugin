...
import { AnswerMatchError, answerMatchKey, bandOrder, candidateTexts, features, fieldClass, hasNegation, metadataReasons, normalizedText, requireText, scopeFingerprint, sensitivity, similarity } from './answer-match-features.js';
function scoreCandidate(question, scope, observedClass, observedSensitivity, record) {
    ...
    if (metadata.some(reason => reason.endsWith('_mismatch')))
        band = 'none';
...
export function rankCandidates(input) {
    ...
    if (limit === null || limit < 1n || limit > 100n)
        throw new AnswerMatchError('candidate limit is invalid');
    if (!Array.isArray(input.candidates))
        throw new AnswerMatchError('candidate collection is invalid');
    ...
    for (const value of input.candidates) {
        ...
        if (seen.has(candidate.key))
            throw new AnswerMatchError('candidate keys are not unique');
        seen.add(candidate.key);
        scored.push(candidate);
    }
    const compare = (a, b) => bandOrder[b.band] - bandOrder[a.band] || b.score - a.score || text(a.key).compare(text(b.key));
    scored.sort(compare);
    if (scored.length) {
        const top = scored[0], positive = (item) => ['exact', 'high'].includes(item.band);
