...
export function proposeCleanup(candidates) {
    if (!Array.isArray(candidates))
        throw new AnswerMatchError('candidate collection is invalid');
    if (!candidates.every((record) => record instanceof PythonObject)) {
    ...
    for (const winner of candidates) {
        ...
        if (!['candidate_active', 'candidate_accepted', 'candidate_confirmed', 'value_seen'].every(reason => policy.includes(reason)))
            continue;
        ...
        for (const duplicate of candidates) {
            ...
            if (duplicateKey === winnerKey)
                continue;
            if (!policyReasons(duplicate).includes('candidate_active'))
                continue;
            if (string(fallback(duplicate, 'reviewStatus', text('accepted'))) !== 'pending')
                continue;
