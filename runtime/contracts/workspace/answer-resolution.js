...
function resolveKey(answers, key) {
...
export function pendingResolutionProjection(field, answers) {
...
export function prepareAnswerResolution(jobs, answers, session, input, preflightReady) {
    ...
    if (jobValue === null || get(object(jobValue, 'job'), 'deletedAt') !== null)
    ...
    const pending = fallback(session, 'pendingFields', []);
    ...
    if (matching.length !== 1)
        throw new JobsError('pending question reference is stale');
    const { field, index } = matching[0];
    const key = string(get(field, 'answerKey'));
    if (!key)
        throw new JobsError('pending question has no referenced answer');
    if (sensitiveField(field))
        throw new JobsError('sensitive pending answers require reconfirmation');
    const resolved = resolveKey(answers, key);
    const answerValue = get(object(get(answers, 'answers'), 'answers'), resolved);
    if (answerValue === null || get(object(answerValue, 'answer'), 'deletedAt') !== null)
        throw new JobsError('referenced answer does not exist');
    const answer = object(answerValue, 'answer');
    if (answerRevision(answer) !== expectedAnswerRevision)
        throw new JobsError('answer revision conflict');
    if (!confirmed(answer))
        throw new JobsError('referenced answer is not accepted and confirmed');
    if (sensitiveAnswer(answer))
        throw new JobsError('sensitive pending answers require reconfirmation');
    const updated = object(parse(serialize(session)), 'session');
    const remaining = get(updated, 'pendingFields').filter((_, position) => position !== index);
    set(updated, 'pendingFields', remaining);
    const blockers = fallback(updated, 'blockers', [])
        .filter(value => string(get(object(value, 'blocker'), 'reference')) !== reference);
    set(updated, 'blockers', blockers);
    const answerKeys = [...fallback(updated, 'answerKeys', [])];
    if (!answerKeys.some(value => string(value) === resolved))
        answerKeys.push(text(resolved));
    ...
    for (const [key, value] of Object.entries({ kind: 'answer_resolution', operationId: input.operationId, jobId, at,
        answerKey: resolved, sourceStatus: 'needs_info', targetStatus: target }))
        set(operation, key, text(value));
    ...
    const pendingInformation = remaining.map(value => {
        ...
        for (const name of ['question', 'state', 'sensitive'])
            if (item.has(text(name)))
                set(projection, name, get(item, name));
        return projection;
    });
