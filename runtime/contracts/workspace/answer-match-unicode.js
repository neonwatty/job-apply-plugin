 ...
 */
export function normalizeMatcherText(value) {
    const result = [];
    let assigned = '';
    for (const char of value) {
        const point = char.codePointAt(0);
        if (contains(point < 0x10000 ? assignedBmp : assignedAstral, point))
            assigned += char;
        else {
            if (assigned)
                result.push(assigned.normalize('NFKC'));
            result.push(char);
            assigned = '';
        }
    }
...
export function matcherTokens(value) {
    const tokens = [];
    let token = '';
    for (const char of value) {
        const point = char.codePointAt(0);
        if (contains(point < 0x10000 ? alnumBmp : alnumAstral, point))
            token += char;
        else if (token) {
            tokens.push(token);
            token = '';
