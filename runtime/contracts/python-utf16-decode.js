...
export function decodePythonUtf16(bytes, start, littleEndian) {
    const points = [];
    const unit = (index) => littleEndian
        ? bytes[index] + bytes[index + 1] * 256 : bytes[index] * 256 + bytes[index + 1];
    for (let index = start; index < bytes.length;) {
        if (index + 1 >= bytes.length) {
            throw new PythonUnicodeDecodeError(littleEndian ? "utf-16-le" : "utf-16-be", bytes, index, bytes.length, "truncated data");
        }
