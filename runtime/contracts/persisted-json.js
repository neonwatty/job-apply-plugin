...
export function* iterPersistedJson(value, options) {
    ...
    function* encode(item, depth) {
        ...
        if (!Array.isArray(item) && !(item instanceof Map))
            throw new TypeError("Object is not JSON serializable");
        if (item instanceof Map ? item.size === 0 : item.length === 0) {
            yield item instanceof Map ? "{}" : "[]";
            return;
        }
        if (active.has(item))
            throw new PersistedJsonError("ValueError", "Circular reference detected");
        active.add(item);
        const indent = "\n" + "  ".repeat(depth + 1);
        if (Array.isArray(item)) {
            ...
            for (let index = 0; index < keys.length; index += 1) {
                if (index > 0)
                    yield "," + indent;
                else if (options.pathProfile !== "3.12")
                    yield indent;
