...
export function pointContents(text) {
...
export class PointSyntaxError extends Error {
    ...
    position;
    trailing;
    constructor(reason, position, trailing) {
        super(reason);
        this.reason = reason;
...
export function scanPointString(points, start) {
    const output = [];
    let position = start + 1;
    while (position < points.length) {
        const point = points[position];
        if (point === 34)
            return { points: output, next: position + 1 };
        if (point < 32)
            throw new PointSyntaxError("Invalid control character at", position);
        if (point !== 92) {
            output.push(point);
            position++;
            continue;
        ...
        if (escaped === 117) {
            ...
            if (high === undefined)
                throw new PointSyntaxError("Invalid \\uXXXX escape", position);
            position += 5;
...
const asciiEscapes = new Map([
    [8, "\\b"], [9, "\\t"], [10, "\\n"], [12, "\\f"], [13, "\\r"], [34, '\\"'], [92, "\\\\"],
]);
const unitEscape = (point) => `\\u${point.toString(16).padStart(4, "0")}`;
export function quotePointString(points) {
    const output = ['"'];
