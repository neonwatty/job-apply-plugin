...
function validateOptions(options) {
    if (options === null || typeof options !== "object")
        throw new TypeError("Expected point JSON options");
...
export function parsePythonPointJson(document, options) {
    validateOptions(options);
    const points = pointContents(document);
    try {
        return parsePointDocument(points, {
            text: points => PythonText.fromCodePoints(points),
            key: points => PythonText.fromCodePoints(points), literal: value => value,
            number: token => {
                try {
                    return parseNumericAtom(token, options);
                }
                catch (error) {
                    if (!(error instanceof NumericAtomError) || error.reason !== "integer-digit-limit")
                        throw error;
        ...
        if (error.trailing && options.diagnosticProfile !== "3.12") {
            throw new PythonPointJsonDecodeError(`Illegal trailing comma before end of ${error.trailing.kind}`, document, error.trailing.comma);
        }
        throw new PythonPointJsonDecodeError(error.reason, document, error.position);
    }
}
export function parsePythonPointJsonBytes(bytes, options) {
    validateOptions(options);
