...
const closedFailureCodes = new Map([
...
function required(request, field) {
    ...
    if (!value)
        throw new JobsError(`native email-only ${field} is invalid`);
    return value;
}
function argumentsFor(request, browserPID) {
    ...
    if (!fingerprint.test(operation))
        throw new JobsError('native email-only operation binding is invalid');
    const revisions = ['jobRevision', 'accountRevision', 'settingsRevision'].map(field => {
        ...
        if (value === null || value < 1n)
            throw new JobsError('native email-only revision binding is invalid');
    ...
    if (controls.some(value => !fingerprint.test(value)))
        throw new JobsError('native email-only control binding is invalid');
    ...
    if (controls[5] !== expectedAggregate)
        throw new JobsError('native email-only aggregate control binding is invalid');
    return ['oracle-email-only-inherited', String(browserPID), portalValue, realm,
        descriptor, ...controls, ...revisions, operation, '3', '4'];
}
function attestation(bytes, operation) {
    if (bytes.length > 4096 || bytes.at(-1) !== 0x0a)
...
export class NativeMacOSEmailOnlyExecutor {
    identity;
    browserProcessIdentifier;
    timeoutMs;
    providerId = 'macos-accessibility';
    constructor(identity, browserProcessIdentifier, timeoutMs = 30000) {
        this.identity = identity;
        this.browserProcessIdentifier = browserProcessIdentifier;
    ...
    static async fromReviewedSources(browserProcessIdentifier, buildDirectory) {
        return new NativeMacOSEmailOnlyExecutor(await buildReviewedOracleHelper(buildDirectory), browserProcessIdentifier);
    }
    async execute(value, privateEmail) {
        ...
        return new Promise((accept, reject) => {
            ...
            const nativeChannel = child.stdio[3], privateChannel = child.stdio[4];
            if (!nativeChannel || !privateChannel) {
                child.kill('SIGKILL');
                reject(new JobsError('native email-only channels are unavailable'));
                return;
            }
            let nativeBytes = Buffer.alloc(0), outputBytes = 0, parsed = null;
            ...
            const timer = setTimeout(() => {
                child.kill('SIGKILL');
                finish(new JobsError('native email-only execution timed out'));
            }, this.timeoutMs);
            for (const stream of [child.stdout, child.stderr])
                stream.on('data', chunk => {
                    ...
                    if (outputBytes > 4096) {
                        child.kill('SIGKILL');
                        finish(new JobsError('native email-only output is invalid'));
                    }
                });
            nativeChannel.on('data', chunk => {
                if (attested) {
                    child.kill('SIGKILL');
                    finish(new JobsError('native email-only attestation is invalid'));
                    return;
                }
                nativeBytes = Buffer.concat([nativeBytes, chunk]);
                if (nativeBytes.length > 4096) {
                    child.kill('SIGKILL');
                    finish(new JobsError('native email-only attestation is invalid'));
                    return;
                }
