...
async function compile(output, sources) {
...
export async function buildReviewedOracleHelper(buildDirectory) {
    ...
    for (const source of sources) {
        ...
        if (!metadata.isFile() || metadata.isSymbolicLink())
            throw new JobsError('reviewed native helper sources are unavailable');
    }
    ...
    if (dirname(output) !== root)
        throw new JobsError('reviewed native helper build path is invalid');
