...
import { FinalActionPolicyService, PolicyError } from '../final-action-policy/service.js';
...
const opaque = (kind, label) => `${kind}:${createHash('sha256').update(label).digest('hex')}`;
const revision = (label) => `sha256:${createHash('sha256').update(label).digest('hex')}`;
async function refused(operation) { try {
    await operation();
    return false;
}
catch (error) {
...
function granted(value) { if (value.mode !== 'auto_submit')
    throw new ReplayError('native policy verification failed'); return value; }
export async function verifyAutoSubmit(path) {
    ...
    if (fixture.steps.filter(step => step.kind === 'review').length !== 1)
    ...
    const service = (name, selectedClock = clock) => new FinalActionPolicyService(join(root, name), { provider, clock: selectedClock });
    ...
    try {
        ...
        const success = service('success');
        await success.activate(campaign);
        const lease = granted(await success.authorize(authorization));
        ...
        await boundary.activate(campaign);
        const boundaryLease = granted(await boundary.authorize(authorization));
        ...
        await boundary.kill();
        const killed = await refused(() => boundary.claim(applicationRef, boundaryLease.leaseId, 1, authorization, capability));
        const expiredAt = new Date(checkedAt.getTime() - 2000), expiry = service('expiry', () => checkedAt);
        const expirySetup = service('expiry', () => expiredAt);
        await expirySetup.activate({ ...campaign, durationSeconds: 1 });
        const expiredLease = granted(await expirySetup.authorize(authorization));
        ...
        await concurrent.activate(campaign);
        const concurrentLease = granted(await concurrent.authorize(authorization));
        ...
        const race = service('race');
        await race.activate(campaign);
        const raceLease = granted(await race.authorize(authorization));
        let release;
        const held = new Promise(accept => { release = accept; }), entered = { value: false };
        ...
        const killing = race.kill();
        let killedEarly = false;
        void killing.then(() => { killedEarly = true; });
        await new Promise(accept => setImmediate(accept));
        const blocked = !killedEarly;
        release();
        await claiming;
        await killing;
        checks['kill-versus-activation-linearized'] = blocked && (await race.status()).mode === 'review_only';
        const limited = service('limit');
        const secondRule = { ...rule, applicationRef: opaque('application', 'second'), urlFingerprint: revision('url-2'), jobFingerprint: revision('job-2') };
        await limited.activate({ ...campaign, applicationRules: [rule, secondRule] });
        await limited.authorize(authorization);
        ...
        await retry.activate(campaign);
        const first = granted(await retry.authorize(authorization));
        const firstClaim = await retry.claim(applicationRef, first.leaseId, first.attempt, authorization, capability);
        const firstReceipt = await retry.outcome(first.campaignId, applicationRef, first.leaseId, firstClaim.claimId, 'uncertain');
        const second = granted(await retry.authorize(authorization));
        const secondClaim = await retry.claim(applicationRef, second.leaseId, second.attempt, authorization, capability);
        const exhausted = await retry.outcome(second.campaignId, applicationRef, second.leaseId, secondClaim.claimId, 'uncertain');
        const denied = await retry.authorize(authorization);
