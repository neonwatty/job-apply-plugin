...
export function artifactBytes(path) {
    ...
    if (bytes.includes(0))
        throw new ArtifactPathValueError();
    return bytes;
}
export function artifactOSError(error) {
...
export async function artifactRoot(root, profile) {
    validatePathProfile(profile);
    if (process.platform === "win32")
        throw new Error("Native Windows artifact verification is not implemented");
    try {
        return filesystemDecode(await withPythonFilesystemErrors(realpath(artifactBytes(root || "."), { encoding: "buffer" })));
    }
    catch (error) {
        if (profile === "3.12" && artifactOSError(error) && error.code === "ELOOP")
            throw new PathLoopError();
        throw error;
    }
}
export async function checkedArtifactPath(root, relative) {
...
export async function regularArtifactFile(root, relative) {
...
export async function artifactIsSymlink(path, profile) {
    try {
        return (await withPythonFilesystemErrors(lstat(artifactBytes(path)))).isSymbolicLink();
    }
    catch (error) {
        if (error instanceof ArtifactPathValueError || error instanceof FilesystemEncodeError)
            return false;
        if (artifactOSError(error) && (profile === "3.14" || error.code === "ENOENT"
            || error.code === "ENOTDIR" || error.code === "EBADF" || error.code === "ELOOP"))
            return false;
        throw error;
    }
}
