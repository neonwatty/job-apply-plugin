...
export class OutcomePolicy extends AuthorizationPolicy {
    async receiptIsLogged(receiptId) {
        if (!await exists(this.receiptsPath))
            return false;
        let lines;
        try {
            lines = (decodePolicyBytes(await readFile(this.receiptsPath))).split(/\r?\n/);
        }
        catch {
            throw new PolicyError('receipt log is unavailable or invalid');
        }
    ...
    async ensureReceiptLogged(receipt) {
        if (await this.receiptIsLogged(receipt.receiptId))
            return;
    ...
    async outcome(campaignId, applicationRef, leaseId, claimId, outcome, confirmationEvent, confirmationCapability) {
        ...
        if (outcome === 'confirmed_submitted') {
            ...
            check(confirmationEvent == null && confirmationCapability == null, 'confirmation evidence is only valid for confirmed submission');
        return this.locked(async () => {
            ...
            if (confirmation)
                check(parseTime(confirmation.observedAt) >= parseTime(attempt.claimedAt) && parseTime(confirmation.observedAt) <= now.getTime(), 'confirmation event time is invalid');
