...
function timeComponents(value, allowEmptyFraction = false) {
...
export function origin(value) {
    ...
    if (host.includes('[') || host.includes(']')) {
        ...
        check(host.split(':')[0].length > 0, 'origin must be an exact HTTP(S) origin');
    return value;
}
...
export function parseSensitive(value) {
...
export function unique(items, key, message) {
...
export const serialization = { pathProfile: '3.12', intMaxStrDigits: 4300 };
export function decodePolicyBytes(bytes) {
...
export function parsePolicyJson(text) {
    function convert(value) {
        ...
        if (value instanceof Map)
            return Object.fromEntries([...value].map(([key, item]) => [key, convert(item)]));
...
export const pythonValue = (value) => parsePythonJson(JSON.stringify(value), serialization);
export const canonical = (value) => serializePythonScope(pythonValue(value));
const ascii = (text) => text.replace(/[\u007f-\uffff]/g, unit => `\\u${unit.charCodeAt(0).toString(16).padStart(4, '0')}`);
export function serializeDocument(value) {
    return [...iterPersistedJson(pythonValue(value), serialization)].map(ascii).join('') + '\n';
}
export function serializeReceiptLine(value) {
...
export const digest = (value) => fingerprint('sha256:' + createHash('sha256').update(canonical(value)).digest('hex'), 'digest');
export function confirmationAuthorityRevision(capability) {
...
export function parseConfirmation(value, claimId, authority, capability) {
...
export function parseReceipt(value) {
    ...
    for (const field of ['slot', 'attempt'])
        check(Number.isSafeInteger(receipt[field]) && Number(receipt[field]) >= 1, `receipt ${field} is invalid`);
    ...
    if (receipt.outcome === 'confirmed_submitted')
        fingerprint(receipt.confirmationRevision, 'confirmationRevision');
    else
        check(receipt.confirmationRevision === null, 'receipt confirmation is invalid');
