...
export function parseCampaign(value) {
    ...
    if (campaign.killSwitchAt !== null)
        parseTime(campaign.killSwitchAt);
...
export function campaignMode(campaign, now) {
    if (campaign.killSwitch)
        return { mode: 'review_only', reason: 'kill_switch' };
    if (campaign.status !== 'active')
        return { mode: 'review_only', reason: campaign.status };
    if (now.getTime() >= parseTime(campaign.expiresAt))
        return { mode: 'review_only', reason: 'expired' };
...
export class CampaignPolicy extends PolicyRepository {
    ...
    async status() {
        ...
        try {
            ...
            return { mode: 'review_only', reason: 'policy_unavailable', campaignId: null };
        }
    }
        ...
        return this.locked(async () => {
            if (await exists(this.campaignPath)) {
                ...
                if (await exists(path)) {
                    ...
                    await this.writeDocument(path, previous);
            }
            await this.writeDocument(this.campaignPath, campaign);
            return campaign;
        });
    }
    async stop(status, kill) {
        const now = this.clock();
        return this.locked(async () => {
            const campaign = await this.loadCampaign();
    ...
    kill() { return this.stop('killed', true); }
    revoke() { return this.stop('revoked', false); }
}
