...
export function authorizationMismatch(campaign, authorization) {
    const rules = campaign.applicationRules.filter(rule => rule.applicationRef === authorization.applicationRef);
    const selected = Object.fromEntries(RULE_FIELDS.map(field => [field, authorization[field]]));
    if (rules.length !== 1 || canonical(rules[0]) !== canonical(selected))
    ...
    if (authorization.answerRevisions.some(item => !allowed.has(item.answerRef) || canonical(allowed.get(item.answerRef)) !== canonical(item)))
        return 'sensitive_scope_mismatch';
    return null;
}
function authorizationResult(application) {
...
export class AuthorizationPolicy extends CampaignPolicy {
    async authorize(incoming) {
        const now = this.clock();
        let authorization;
        try {
            authorization = parseAuthorization(incoming);
        }
        catch {
        ...
        return this.locked(async () => {
            try {
                ...
                if (decision.mode !== 'auto_submit')
                    return { mode: 'review_only', reason: decision.reason };
                ...
                if (await exists(path)) {
                    ...
                    if (application.authorizationFingerprint !== authorizationFingerprint)
                        return { mode: 'review_only', reason: 'authorization_changed' };
                    if (application.status === 'lease_issued') {
                        if (now.getTime() >= parseTime(application.attempts.at(-1).expiresAt))
    ...
    async claim(applicationRef, leaseId, attemptOrdinal, observedAuthorization, actionCapability, activation) {
        const now = this.clock();
        ...
        return this.locked(async () => {
            ...
            const claim = { mode: 'auto_submit', reason: 'atomic_action_claim', campaignId: campaign.campaignId,
