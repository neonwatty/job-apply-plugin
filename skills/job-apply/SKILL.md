...
- Ordinary application: after Store initialization, read [canonical intake](references/intake.md), then [filling and handoff](references/application.md). Start or reuse an application run whose managed resume and confirmed facts were explicitly confirmed in chat. A supplied URL is ingested before browser work and added to that run's queue.
