...
Let the owner select any returned jobs for the queue. For selected results or an explicit request to import old reports, read [queue intake](references/queue.md). Preview the exact selected changes and commit only the authorized items through the canonical Store. Reuse an explicit request to save that selection; ask for a new choice only when the preview exceeds its scope. After intake, automatically run the readiness checks described in queue intake. Do not auto-select jobs by position or invent a minimum quality threshold. Report the helper's committed results and conflicts; search completion does not imply application submission.
