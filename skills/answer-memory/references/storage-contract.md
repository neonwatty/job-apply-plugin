...
```bash
node "<plugin-root>/apps/companion/command.mjs" store --help
```
...
Internal protocol: the private attempt broker calls `job-acquire`; ordinary agents use `node "<plugin-root>/apps/companion/command.mjs" attempt` as described in [canonical intake](../../job-apply/references/intake.md), never the raw claim commands below. One version-1 `coordinator.json` record holds at most one claim with a hashed token and a 300-second lease; `coordinator-journal.json` provides idempotent crash roll-forward. Neither file is a second workflow-status model: `jobs.json` remains authoritative.
...
Use `claim-progress` for value-free checkpoints. Use `claim-handoff --status needs_info` for a blocked application and `claim-handoff --status awaiting_review` at final review. The latter is accepted only with a Store-recomputed complete `agent_attested_current_attempt` readiness report bound to the exact post-acquisition/recovery revision, a complete observed required-control manifest that exactly matches the selected bundled fixture, no unresolved fields or blockers, and the exact owner-review handoff. An additional required control or the absence of an exact fixture fails closed. The attestation describes the agent's current visible-form observation; it is not independent proof of browser provenance. Replay-only, stale, inaccessible, incomplete, rejected, validation-error, upload-fallback, or final-action-activated evidence fails without mutation. Each handoff atomically journals the status transition, session, lifecycle history, and claim release. Durable sessions strip question/role/company/URL copies and retain only allowlisted typed blockers, field-specific approvals, readiness, and browser-handoff state. Claims, journals, sessions, and history must never contain raw tokens, applicant values, resume contents or identity, credentials/codes, URLs/paths, tab IDs, or browser state.
...
Semantic lookup is a read-only adapter over the frozen deterministic matcher. The Store loads current answer records, ranks exact/alias/semantic candidates, and evaluates `strict` or `bounded_loose` policy server-side. Results contain only opaque keys, confidence bands, and closed reason codes. Automatic reuse requires exact scope, field class and sensitivity compatibility, an active accepted confirmed seen value, eligible confidence without a tie, and explicit current-use authority. Sensitive bounded policy additionally requires an explicit field-class allowlist. It never grants remember or final-action authority. Grouped approval is revision- and preview-bound and preserves `currentUse`, `remember`, `policyMode`, and `useAuthority` independently per field.

Cleanup is always two-phase. Preview recomputes unambiguous accepted-winner/pending-duplicate proposals without mutation. Approval requires the exact preview token, both exact revisions, the selected pair, and explicit owner confirmation; drift rejects before the existing crash-safe merge journal. No automatic cleanup occurs.
