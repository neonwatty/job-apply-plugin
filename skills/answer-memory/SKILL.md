...
python3 "<plugin-root>/scripts/job-apply-store.py" job-review-restart --id <job-id> --owner <owner-label> --expected-revision <revision> --owner-confirmed-not-submitted
...
`job-acquire` requires the revision shown during selection, rechecks preflight under the global lock, and returns the resolved resume record, preferring the job's assigned resume over the active default. A separate `job-review-restart` operation exists only for an exact `awaiting_review` revision after the owner explicitly confirms that they did not submit it. It requires the same job's complete reviewed session and reviewed history, no pending work or global claim, and a fresh current managed-resume preflight. The modern path requires all prior readiness metadata exactly as recorded. A one-time legacy rebuild is eligible only when `attemptRevision`, `readiness`, and `browserHandoff` are all structurally absent from a `review`/`final_review` session, the latest same-job event is `reviewed`/`awaiting_review`, and no prior restart exists. Partial, explicit-null, malformed, or contradictory metadata fails closed. The operation atomically advances that same job once to `in_progress`, creates one 300-second claim, records one value-free `job-restarted` or `legacy-review-rebuild` event, and leaves the prior review-session bytes unchanged until claim-gated progress. Legacy readiness is unknown rather than inferred, so a rebuilt attempt must provide fresh current-attempt readiness and browser handoff before review. It never makes `awaiting_review` equivalent to `ready` and grants no browser or final-action authority. Terminal handoff likewise requires the retained post-acquisition/recovery/restart revision; reloading just before handoff would mask a concurrent change. `awaiting_review` additionally requires a Store-recomputed, complete `agent_attested_current_attempt` readiness report bound to that revision, with a complete observed required-control manifest exactly matching the selected bundled fixture, no unresolved fields or blockers, and the exact owner-review handoff. An additional required control or the absence of an exact fixture fails closed. This is an agent attestation over closed current-form observations, not independent browser-provenance proof. Replay evidence, stale/inaccessible controls, unresolved validation, upload fallback, or final-action activation rejects without mutation. A live claim is never stolen or silently released. Generic transition, trash, deletion, and session mutation reject active canonical jobs. Expired claims require explicit same-job recovery. Both handoffs and review restart journal and roll forward job, history, value-free session, and claim state idempotently after a crash.
