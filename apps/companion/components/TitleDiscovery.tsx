...
import { copy, get, set, string, text, type Value } from '../../../src/contracts/workspace/values';
...
export function TitleDiscovery({ client, onSaved, dirtyChanged }: { client: Client; onSaved: (snapshot: ProfileSnapshot) => void; dirtyChanged: (dirty: boolean) => void }) {
  ...
  return <section className="workspace-panel title-discovery" id="title-discovery" aria-labelledby="title-discovery-title">
    ...
    {open && snapshot && <div className="title-discovery-review">
      ...
      <label>Title discovery JSON result packet<textarea value={packetText} onChange={event => { setPacketText(event.target.value); setPacket(null); setChoices(choicesFor(snapshot)); }} rows={5} /></label>
      ...
      <fieldset disabled={busy}><legend>Choose exact target titles</legend>
        {choices.map(choice => <div className="title-discovery-choice" key={choice.id}>
          ...
          <button type="button" className="text-action" onClick={() => setChoices(value => value.filter(item => item.id !== choice.id))}>Remove {choice.title || 'title'}</button>
        </div>)}
        <button type="button" className="secondary" onClick={() => setChoices(value => [...value, { id: `manual-${crypto.randomUUID()}`, title: '', selected: true, existing: false }])}>Add title manually</button>
      </fieldset>
