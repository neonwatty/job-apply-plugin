...
test('packaged title discovery installed skills drive Companion with host lanes truthfully unrun', { timeout: 120_000 }, async (t) => {
  ...
  const codexHost = process.env.JOB_TITLE_DISCOVERY_CODEX_HOST;
  const claudeHost = process.env.JOB_TITLE_DISCOVERY_CLAUDE_HOST;
  if (!codexHost && !claudeHost) {
    t.skip('isolated installed Codex and Claude roots are provided by plugin smoke');
    return;
  }
  assert.deepEqual(await packagedTitleDiscoveryJourney({ codexRoot, claudeConfigRoot: claudeConfig, codexHost, claudeHost }),
    { codex: { installed: Boolean(codexRoot), agent: codexHost },
      claude: { installed: Boolean(claudeConfig), agent: claudeHost },
