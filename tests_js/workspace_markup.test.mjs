...
test("job-apply routes every ordinary URL through the canonical task protocol", async () => {
  ...
  assert.match(skill, /command\.mjs" task[\s\S]{0,300}intake --input/);
  assert.match(skill, /command\.mjs" task \[--root <resolved-root>\] snapshot/);
  ...
  assert.match(readme, /packaged native `task` surface/);
});
