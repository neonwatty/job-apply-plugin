...
const contractsSource = await readFile(new URL('../apps/companion/components/contracts.ts', import.meta.url), 'utf8');
const contracts = data(emit(contractsSource).replace(/'\.\.\/\.\.\/\.\.\/src\/([^']+)'/g, (_match, path) =>
...
const job = { id: 'job-one', revision: 7n, status: 'saved', url: 'https://example.org/job' };
...
test('payload binds current revision and rejects unsupported transitions or malformed identity', () => {
  ...
  for (const revision of [0n, -1n, 1.5, Number.MAX_SAFE_INTEGER + 1, NaN])
...
test('acknowledgement must decode a full job with matching identity, target, and advanced revision', () => {
  const result = { ...job, revision: 8n, status: 'ready', notes: 'Preserved' };
  const raw = '{"id":"job-one","revision":8,"status":"ready","url":"https://example.org/job","notes":"Preserved"}';
  assert.deepEqual(model.transitionAcknowledgement(raw, job, 'ready'), result);
  for (const bad of [{ ...result, id: 'other' }, { ...result, revision: 7n },
    { ...result, status: 'saved' }, { status: 'ready' }, { job: result }]) {
    const encoded = JSON.stringify(bad, (_key, value) => typeof value === 'bigint' ? Number(value) : value);
    assert.throws(() => model.transitionAcknowledgement(encoded, job, 'ready'));
  }
