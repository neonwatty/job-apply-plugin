...
test('synthetic PDF has an authenticated browser preview', { timeout: 60_000 }, async () => {
  ...
  try {
    ...
    await page.locator('#nav-resumes').click();
...
for (const width of [1280, 390]) {
  test(`grouped navigation stays visible at ${width}px`, { timeout: 60_000 }, async () => {
    ...
    try {
      ...
      assert.ok(navigation.y >= 0 && navigation.y + navigation.height <= 844, 'primary navigation remains in viewport');
      for (const id of ['nav-group-pipeline', 'nav-group-application-data', 'nav-group-controls']) {
        const box = await page.locator(`#${id}`).boundingBox();
        assert.ok(box.x >= 0 && box.x + box.width <= width, `${id} needs no horizontal scroll`);
      }
