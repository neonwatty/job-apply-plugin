...
function verify(executable, t) {
  ...
  const expected = composed.profiles.find(profile => profile.version === selected.python);
  const extra = supplemental.profiles.find(profile => profile.receipt.profile.version === selected.python)?.receipt;
  assert.ok(expected && extra, `Unreviewed CPython patch ${selected.python}`);
  ...
  // Historical bindings remain immutable; relocated equal bytes are admissible,
  // whereas a different executable or JSON implementation is not silently reused.
  assert.equal(extra.profile.executable.sha256, actual.profile.executableSha256);
  assert.equal(extra.profile.machine, selected.machine);
  for (const [name, binding] of Object.entries(extra.stdlib)) assert.equal(binding.sha256, actual.stdlib[name].sha256);
  for (const [name, binding] of Object.entries(extra.native)) {
    assert.equal(binding.sha256, actual.native[name].sha256);
    assert.equal(binding.origin === 'built-in', actual.native[name].origin === 'built-in');
  }
  ...
  for (const row of expected.cases) assert.deepEqual(actual.cases.find(item => item.id === row.id), row, row.id);
  assert.deepEqual(actual.serializers, composed.serializers);
  for (const row of expected.cases) assert.deepEqual(observeTs(row, selected.python), row.outcome, row.id);
  ...
  assert.deepEqual(observedExtra.cases, extra.cases);
  for (const row of extra.cases) assert.deepEqual(observeTs(row, selected.python), row.outcome, row.id);
  t.diagnostic(JSON.stringify({ ...actual.profile, stdlib: actual.stdlib, native: actual.native,
    composedCases: 38, supplementalCases: 48, serializerGraphs: 3, defaultMayDuplicateProfile: executable === 'python3' }));
}

test('S08 point JSON matches the frozen composed reference under default CPython', t => verify('python3', t));
test('S08 point JSON matches the frozen composed reference under CPython 3.12', t => verify('python3.12', t));
test('S08 point JSON matches the frozen composed reference under CPython 3.13', t => verify('python3.13', t));
test('S08 point JSON matches the frozen composed reference under CPython 3.14', t => verify('python3.14', t));
