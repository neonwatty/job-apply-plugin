...
type Activity = {
  ...
  claim?: { state?: unknown; heartbeatAt?: unknown } | null;
  history?: { length?: unknown } | null;
};
type AttentionItem = { jobId?: unknown; reasonCode?: unknown; missingInformationCount?: unknown;
  session?: { blockers?: { type?: unknown; code?: unknown }[] | null } | null };
type Projection = { items?: AttentionItem[] | null };

export function filterJobs(jobs: Job[], query: string = "", status: unknown = "") {
...
export function transitionsFor(status: PropertyKey): unknown {
  return ({
  ...
  } as Record<PropertyKey, unknown>)[status] || [];
}

export function canMarkReadyFrom(status: unknown): boolean {
...
export function activitySignature(activity: Activity | null | undefined): string {
  if (!activity) return "";
...
export function activityAnnouncement(previous: Activity | null | undefined, current: Activity): string {
...
export function attentionMembershipSignature(projection: Projection | null | undefined): string {
  return JSON.stringify((projection?.items || []).map((item) => [item.jobId, item.reasonCode]));
}

export function attentionAnnouncement(previous: Projection | null | undefined, current: Projection | null | undefined): string {
...
export function attentionMissingInformationText(item: AttentionItem | null | undefined): string {
...
export function attentionBlockerSummary(item: AttentionItem | null | undefined): string {
...
export function ownerBetaNextStep(action: PropertyKey): unknown {
  return ({
  ...
  } as Record<PropertyKey, unknown>)[action] || ["Review the workspace", "Refresh the canonical Store and choose a workspace section."];
}
