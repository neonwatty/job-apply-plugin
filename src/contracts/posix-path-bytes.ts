...
export function filesystemEncode(value: string): Buffer {
  const chunks: Buffer[] = [];
  let start = 0;
  for (let index = 0; index < value.length; index += 1) {
    ...
    if (unit < 0xdc80 || unit > 0xdcff) throw new FilesystemEncodeError();
...
export function filesystemDecode(bytes: Uint8Array): string {
  const output: string[] = [];
  let index = 0;
  while (index < bytes.length) {
    ...
    if ((first === 0xe0 && second! < 0xa0) || (first === 0xed && second! >= 0xa0)
      || (first === 0xf0 && second! < 0x90) || (first === 0xf4 && second! >= 0x90)) valid = false;
    if (!valid) {
