...
export function decodePythonUtf8(bytes: Buffer): number[] {
  const points: number[] = [];
  ...
  for (let index = 0; index < bytes.length;) {
    ...
    if (first < 0x80) { points.push(first); index++; continue; }
    const width = first >= 0xc2 && first <= 0xdf ? 2
      : first >= 0xe0 && first <= 0xef ? 3
      : first >= 0xf0 && first <= 0xf4 ? 4 : 0;
    if (!width) fail(index, index + 1, "invalid start byte");
    ...
    for (let offset = 1; offset < width; offset++) {
      ...
      if (byte === undefined) fail(index, index + offset, "unexpected end of data");
      const lower = offset === 1 && first === 0xe0 ? 0xa0
        : offset === 1 && first === 0xf0 ? 0x90 : 0x80;
      const upper = offset === 1 && first === 0xf4 ? 0x8f : 0xbf;
      if (byte! < lower || byte! > upper) fail(index, index + offset, "invalid continuation byte");
