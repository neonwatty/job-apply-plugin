...
export function filesystemEncodePoint(text: PythonText): Buffer {
  ...
  for (let index = 0; index < points.length; index++) {
    ...
    if (point >= 0xd800 && point <= 0xdfff && (point < 0xdc80 || point > 0xdcff)) {
      let end = index + 1;
      while (end < points.length && points[end]! >= 0xd800 && points[end]! <= 0xdfff) end++;
...
export function filesystemDecodePoint(bytes: Uint8Array): PythonText {
