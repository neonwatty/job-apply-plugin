/** Exact rational arithmetic for Python's shortest, nearest binary64 spelling. */
...
function exactMagnitude(value: number): Rational {
...
function comparePower(value: Rational, exponent: number): number {
...
function decimalExponent(value: Rational, magnitude: number): number {
  // log10 is only a starting estimate; exact comparisons resolve power boundaries.
  let exponent = Math.floor(Math.log10(magnitude));
  while (comparePower(value, exponent) < 0) exponent -= 1;
  while (comparePower(value, exponent + 1) >= 0) exponent += 1;
  return exponent;
}

function decimalComponents(magnitude: number): { digits: string; exponent: number } {
  ...
  for (let precision = 1; precision <= 17; precision += 1) {
    ...
    for (const candidate of [floor, floor + 1n]) {
      if (Number(`${candidate}e${scale}`) !== magnitude) continue;
...
export function floatScope(value: number): string {
