...
export const passwordlessEmailCodeFlow = 'passwordless_email_code';
export const accountNotRequiredFlow = 'account_not_required';
...
function resolved(adapterId: string, descriptor: string, authorityKind: string): Realm {
  const flowKind = adapterId === 'workday' ? passwordFlow
    : adapterId === 'mygreenhouse' ? passwordlessEmailCodeFlow : emailFlow;
  return { status: 'resolved', adapterId, descriptorVersion: 1, descriptor,
    ...
    flowKind, credentialRequired: adapterId === 'workday' };
}
...
export function resolveAccountRealm(input: unknown): Realm {
  ...
  if (host === 'my.greenhouse.io') {
    const path = parsed[3]!;
    if (parsed[4] || path.includes('%') || path.includes('//')) return unresolved('mygreenhouse_path_unproven');
    return resolved('mygreenhouse', 'mygreenhouse:v1:global', 'global');
  }
...
export type AccountFlow = Realm | (Omit<Extract<Realm, {status: 'resolved'}>, 'status'> & {
  status: 'classified'; accountRequired: boolean;
}) | { status: 'classified'; adapterId: 'greenhouse'; flowKind: typeof accountNotRequiredFlow;
  credentialRequired: false; accountRequired: false };
export function classifyAccountFlow(input: unknown): AccountFlow {
  const realm = resolveAccountRealm(input);
  if (realm.status === 'resolved') return {...realm, status: 'classified',
    accountRequired: realm.adapterId !== 'mygreenhouse'};
  if (typeof input !== 'string') return realm;
  const value = strip(input).replace(/^[\x00-\x20]+/, '').replace(/[\t\r\n]/g, '');
  const parsed = /^https:\/\/([^/?#]+)([^?#]*)(?:\?([^#]*))?(?:#(.*))?$/is.exec(value);
  if (!parsed || parsed[1]!.includes('@') || parsed[3] || parsed[4]) return realm;
  const authority = /^([^:]+)(?::([0-9]+))?$/.exec(parsed[1]!.toLowerCase());
  if (!authority || authority[2] !== undefined && BigInt(authority[2]) !== 443n) return realm;
  const host = authority[1]!;
  if (!['boards.greenhouse.io', 'job-boards.greenhouse.io'].includes(host)
    || !/^\/[a-z0-9](?:[a-z0-9_-]{0,126}[a-z0-9])?\/jobs\/[1-9][0-9]*\/?$/.test(parsed[2]!)) return realm;
  return {status: 'classified', adapterId: 'greenhouse', flowKind: accountNotRequiredFlow,
    credentialRequired: false, accountRequired: false};
}
