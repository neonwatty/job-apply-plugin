...
export function validateAccount(key: string, value: Value): Document {
  ...
  if (expectedFlow === null || flow !== expectedFlow
    ...
    || createHash('sha256').update(descriptor).digest('hex') !== key) throw new JobsError('employer account realm descriptor is invalid');
  ...
  } else if (flow === emailFlow || flow === passwordlessEmailCodeFlow) throw new JobsError('provider-free account cannot have protected credential metadata');
