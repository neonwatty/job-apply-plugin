...
const id = (value: Value, label: string): string => {
  const result = string(value);
  if (!result || !idPattern.test(result) || result.includes('..')) throw new JobsError(`${label} is invalid`);
  return result;
};

export function validateRunSelection(value: Value): Document {
...
function validateQueueVersion(value: Value, previous: bigint): bigint {
  ...
  if (new Set(values).size !== values.length) throw new JobsError('application run queue contains duplicate jobs');
  return revision;
}

export function validateApplicationRun(key: string, value: Value): Document {
  const run = object(value, 'application run');
  exact(run, ['runId', 'status', 'revision', 'selection', 'queueVersions', 'createdAt', 'updatedAt', 'completedAt'],
    'application run is invalid');
  if (id(get(run, 'runId'), 'application run id') !== key) throw new JobsError('application run id does not match its index');
  ...
  for (const version of versions) queueRevision = validateQueueVersion(version, queueRevision);
  if (revision < queueRevision || (status === 'active' && revision !== queueRevision)) throw new JobsError('application run revision is invalid');
  if ((status === 'completed') !== Boolean(string(get(run, 'completedAt')))) throw new JobsError('application run completion is invalid');
  return run;
}

export function validateApplicationRuns(metadata: Document): Document | null {
  ...
  for (const [key, value] of runs.entries()) {
    ...
    if (string(get(run, 'status')) === 'active') {
      if (active !== null) throw new JobsError('multiple application runs are active');
  ...
  if (activeRunId !== active) throw new JobsError('active application run reference is invalid');
  return document;
}

export function activeApplicationRun(jobs: Document): Document | null {
  const metadata = get(jobs, 'metadata');
  if (metadata === null) return null;
  const runs = validateApplicationRuns(object(metadata, 'jobs metadata'));
  if (runs === null || get(runs, 'activeRunId') === null) return null;
  return object(get(object(get(runs, 'runs'), 'application runs'), string(get(runs, 'activeRunId'))!), 'active application run');
}

export function currentRunJobIds(run: Document): string[] {
  const versions = get(run, 'queueVersions') as Value[];
  return (get(object(versions[versions.length - 1]!, 'application run queue version'), 'jobIds') as Value[]).map(item => string(item)!);
}
