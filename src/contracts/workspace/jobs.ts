...
export function validateInput(value: Value, patch: boolean): Document {
...
export function validateJob(key: string, value: Value): Document {
  const record = object(value, "job record");
  if (keys(record).some(key => !recordFields.has(key))) throw new JobsError("job record contains unsupported fields");
  if (string(get(record, "id")) !== key) throw new JobsError("job record id does not match its index");
  safeId(key);
  ...
  } else if (outcome !== null) throw new JobsError("open job cannot have a closed outcome");
  const priority = has(record, "priority") ? int(get(record, "priority")) : 0n;
  if (priority === null || priority < 0n || priority > 5n) throw new JobsError("job priority must be an integer from 0 to 5");
  ...
  for (const value of sources) {
    ...
    if (string(get(source, "sourceKind")) !== "timestamped-search-report") throw new JobsError("job legacy source kind is unsupported");
    const path = string(get(source, "relativePath"));
    if (!path || path.includes("/") || !path.startsWith("search-") || !path.endsWith(".md")) throw new JobsError("job legacy source path is invalid");
    if (!/^legacy-entry-[0-9a-f]{24}$/.test(string(get(source, "entryId")) ?? "")) throw new JobsError("job legacy source entry id is invalid");
  ...
  for (const [field, label] of [["createdAt", "creation"], ["updatedAt", "update"]]) {
    if (!string(get(record, field!))) throw new JobsError(`job record has no ${label} timestamp`);
  }
  return record;
}
export function validateJobsDocument(value: Value): Document {
...
export const emptyObject = (): Document => new PythonObject<Value>();
