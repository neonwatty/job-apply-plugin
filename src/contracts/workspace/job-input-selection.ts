...
export function validateJobInputSelection(value: Value): Document {
  const record = object(value, 'job input selection');
  exact(record, ['resumeId', 'contentRevision', 'factRevision', 'jobRevision', 'confirmedAt'],
    'job input selection is invalid');
  ...
  for (const field of ['factRevision', 'jobRevision']) {
    ...
    if (revision === null || revision < 1n) throw new JobsError('job input selection revision is invalid');
  }
  if (!string(get(record, 'confirmedAt'))) throw new JobsError('job input confirmation time is invalid');
  return record;
}
