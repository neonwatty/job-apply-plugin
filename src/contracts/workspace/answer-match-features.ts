...
export const bandOrder: Readonly<Record<string, number>> = { exact:3, high:2, uncertain:1, none:0 };
export function requireText(value: Value, label: string): string {
  const result = string(value);
  if (result === null || !strip(result)) throw new AnswerMatchError(`${label} is invalid`);
  return result;
}
export function answerMatchKey(record: Document): string {
  return requireText(fallback(record,'answerKey',get(record,'key')),'answer key');
}
export function fieldClass(value: Value): string {
  const result = string(value);
...
export function sensitivity(value: Value): string {
  const result = string(value);
  if (!['none','personal','high'].includes(result!)) throw new AnswerMatchError('sensitivity is invalid');
  return result!;
}
export function scopeFingerprint(value: Value): string {
  if (!(value instanceof PythonObject)) throw new AnswerMatchError('scope is invalid');
  try { return serialize(value); }
  catch { throw new AnswerMatchError('scope is invalid'); }
}
export function rawTokens(question: string): string[] {
  const normalized = casefold(normalizeMatcherText(requireText(text(question),'question')));
  return matcherTokens(normalized);
}
export const normalizedText = (question: string): string => rawTokens(question).join(' ');
export const hasNegation = (question: string): boolean => rawTokens(question).some(token => negations.has(token));
export function features(question: string): Set<string> {
  ...
  for (const token of rawTokens(question)) {
    ...
    if ([...canonical].length > 2) result.add(canonical);
  }
  return result;
}
export function similarity(left: Set<string>, right: Set<string>): [number, number] {
  const shared = [...left].filter(item => right.has(item)).length;
  if (!left.size || !right.size || !shared) return [0,0];
  return [0.65 * (shared / Math.min(left.size,right.size)) + 0.35 * (shared / new Set([...left,...right]).size),shared];
}
export function candidateTexts(record: Document): string[] {
  const question = requireText(get(record,'question'),'candidate question');
  const aliases = fallback(record,'aliases',[]);
  if (!Array.isArray(aliases) || aliases.some(value => string(value) === null || !strip(string(value)!))) throw new AnswerMatchError('candidate aliases are invalid');
  return [question,...aliases.map(value => string(value)!)];
}
export function metadataReasons(record: Document, scope: Value, observedClass: string, observedSensitivity: string): string[] {
  const candidateSensitivity = string(fallback(record,'sensitivity',text('none')));
  if (!['none','personal','high'].includes(candidateSensitivity!)) throw new AnswerMatchError('candidate sensitivity is invalid');
