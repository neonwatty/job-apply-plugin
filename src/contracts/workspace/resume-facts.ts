...
export type ResumeFactState = 'draft' | 'confirmed';

/** A resume's fact versions are immutable. Staleness is derived from the file. */
export function validateResumeFactVersion(value: Value, expectedRevision: bigint): Document {
  const record = object(value, 'resume fact version');
  exact(record, ['revision', 'contentRevision', 'state', 'facts', 'createdAt', 'confirmedAt'],
    'resume fact version contains unsupported fields');
...
export function validateResumeFacts(document: Document): Document {
