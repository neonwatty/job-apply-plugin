...
function positive(binding: Document, field: string): bigint {
  const value = int(get(binding, field));
  if (value === null || value < 1n) throw new JobsError('canary revision binding is invalid');
  return value;
}

export function validateOracleCanaryBinding(value: Value): Document {
  ...
  for (const field of ['jobRevision', 'accountRevision', 'settingsRevision', 'approvalRevision']) positive(binding, field);
  for (const field of ['portalFingerprint', 'portalNameFingerprint', 'accountCreationControlsFingerprint', ...componentFields]) {
    if (!fingerprintPattern.test(string(get(binding, field)) ?? '')) throw new JobsError('canary fingerprint binding is invalid');
  }
...
function scalar(binding: Document, key: string): string {
...
export function validateOracleCanaryFinalScope(value: Value): Document {
...
function domainDigest(domain: string, binding: Document, omitClaim: boolean): string {
  if (omitClaim && !has(binding, 'claimId')) validateOracleCanaryFinalScope(binding);
  else validateOracleCanaryBinding(binding);
...
export const canaryBindingDigest = (binding: Document): string => domainDigest('claim-bound-execution', binding, false);
export const canaryFinalScopeDigest = (binding: Document): string => domainDigest('final-owner-approval', binding, true);
export const privateCanaryDigest = (value: string): string => `sha256:${createHash('sha256').update(value, 'ascii').digest('hex')}`;

export interface ValidatedLiveEmailCanary { capabilityRef: string; binding: Document; packet: Document }

export function validateLiveEmailCanaryRequest(value: Value): ValidatedLiveEmailCanary {
  ...
  if (!capabilityPattern.test(capabilityRef)) throw new JobsError('live canary capability is invalid');
  ...
  let portal: URL;
  try { portal = new URL(portalUrl); } catch { throw new JobsError('live canary portal is invalid'); }
  ...
  if (realm.status !== 'resolved' || realm.adapterId !== 'oracle-recruiting' || realm.flowKind !== emailFlow
    || realm.realmRef !== string(get(binding, 'realmRef'))) throw new JobsError('email-only canary realm binding is invalid');
