...
import {
  agentCodes, fields, matches, member, optionalStrings, positive, readinessCodes,
  requireCondition as check, validateApprovals, validateBlockers, validatePendingFields,
} from './answer-session-fields.js';

export function safeAnswerSessionId(value: Value): string {
  const id = string(value);
  check(matches(value, /^[A-Za-z0-9][A-Za-z0-9._-]{0,127}$/u) && !id!.includes('..'), 'application id contains unsupported characters');
  return id!;
}
function validateReadiness(value: Value, attempt: Value): void {
  ...
  const codes = (blockers as Value[]).map(string);
...
function validateHandoff(value: Value): void {
...
function projectLegacyPending(applicationId: string, value: Value, ats: Value): Document {
  let field: Document;
  try { field = object(value, 'pending field'); }
  catch { throw new JobsError('pending field must be a JSON object'); }
...
export function projectStoredAnswerSession(value: Value, expectedId?: string, expectedAts?: string | null): Document {
  ...
  if (!legacy) {
    ...
    check(expectedId === undefined || string(get(validated, 'applicationId')) === expectedId,
      'session application id does not match path');
    return validated;
  }
  check(!pending.some(item => has(object(item, 'pending field'), 'reference')),
    'legacy and modern pending fields cannot be mixed');
...
export function validateAnswerHistory(value: Value): Document {
...
function validateVersion(document: Document, label: string): void {
