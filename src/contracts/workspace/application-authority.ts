...
export function applicationOrigin(value: Value): string {
  const raw = string(value);
  if (raw === null) throw new JobsError('application destination is invalid');
  const normalized = normalizeJobUrl(raw);
  const match = /^(https?):\/\/([^/?#]+)/u.exec(normalized);
  if (!match) throw new JobsError('application destination is invalid');
  return `${match[1]}://${match[2]}`;
}

function positive(value: Value, label: string, zero = false): bigint {
  const result = int(value);
  if (result === null || result < (zero ? 0n : 1n)) throw new JobsError(`${label} must be a positive integer`);
  return result;
}

function strings(value: Value, label: string, allowed?: Set<string>): string[] {
  ...
  if (result.some(item => item === null || !item) || new Set(result).size !== result.length
    || allowed && result.some(item => !allowed.has(item!))) throw new JobsError(`${label} is invalid`);
  return result as string[];
}

export function validateApplicationAuthorityBinding(value: Value): Document {
  const binding = object(value, 'application authority job binding');
  exact(binding, ['jobId', 'destinationOrigin', 'resumeId', 'resumeRevision', 'contentRevision', 'factRevision'],
    'application authority job binding contains unsupported fields');
  safeId(string(get(binding, 'jobId'))); safeId(string(get(binding, 'resumeId')));
...
export function validateSensitiveAnswerBinding(value: Value): Document {
  ...
  if (!string(get(binding, 'answerRef'))) throw new JobsError('sensitive answer reference is invalid');
...
export function validateApplicationAuthority(value: Value): Document {
  ...
  if (active ? terminal !== null : !string(terminal)) throw new JobsError('application authority terminal timestamp is invalid');
  const bindings = get(record, 'jobBindings');
  if (!Array.isArray(bindings) || bindings.length === 0 || bindings.length > 50) throw new JobsError('application authority job scope is invalid');
  ...
  if (!Array.isArray(sensitive)) throw new JobsError('sensitive answer bindings must be a list');
  const answers = sensitive.map(validateSensitiveAnswerBinding).map(item => string(get(item, 'answerRef'))!);
  if (new Set(answers).size !== answers.length) throw new JobsError('sensitive answer bindings contain duplicates');
  return record;
}

export function initialApplicationAuthorityDocument(now: string): Document {
  claimTime(now);
...
export function validateApplicationAuthorityDocument(value: Value): Document {
  const document = object(value, 'application authorities');
  exact(document, ['schemaVersion', 'activeAuthorityId', 'authorities', 'metadata'],
    'application authority document contains unsupported fields');
  if (int(get(document, 'schemaVersion')) !== 1n) throw new JobsError('application authority schema version is unsupported');
  ...
  for (const [key, value] of authorities.entries()) {
    ...
    if (string(key) !== string(get(record, 'authorizationId'))) throw new JobsError('application authority identity does not match');
  }
  ...
  for (const field of ['createdAt', 'updatedAt']) if (!string(get(metadata, field))) throw new JobsError('application authority metadata is invalid');
  return document;
}

export function activeApplicationAuthority(document: Document): Document | null {
...
export function applicationAuthorityProjection(document: Document, now: string): Document {
  ...
  if (record === null) return set(object(fromJSON({ mode: 'guided', status: 'active', authorizationId: null,
  ...
  const result = object(fromJSON({ mode: expired ? 'guided' : string(get(record, 'mode')),
    ...
    expiresAt: string(get(record, 'expiresAt')), runId: string(get(record, 'runId')), jobIds: [], sensitiveAnswerRefs: [] }),
  'application authority projection');
  set(result, 'revision', revision);
  set(result, 'jobIds', (get(record, 'jobBindings') as Value[]).map(item => get(object(item, 'job binding'), 'jobId')));
  set(result, 'sensitiveAnswerRefs', (get(record, 'sensitiveAnswerBindings') as Value[]).map(item => get(object(item, 'answer binding'), 'answerRef')));
  return result;
}

export function parseApplicationAuthorityOperations(value: Value): string[] {
  return strings(value, 'application authority operations', applicationAuthorityOperations);
}

export function applicationAuthorityInterrupt(value: Value): string | null {
  ...
  if (interrupts.size !== applicationAuthorityInterrupts.size || keys(interrupts).some(key => !applicationAuthorityInterrupts.has(key))
    || keys(interrupts).some(key => typeof get(interrupts, key) !== 'boolean')) throw new JobsError('application authority interrupts are invalid');
  for (const [field, reason] of applicationAuthorityInterrupts) if (get(interrupts, field) === true) return reason;
  return null;
}

export function validateBoundSensitiveAnswer(record: Document, binding: Document): boolean {
  return answerRevision(record) === int(get(binding, 'answerRevision'));
}

export function replaceAuthority(document: Document, record: Document, now: string): Document {
  const result = copy(document), previous = activeApplicationAuthority(result);
  if (previous !== null) {
    set(previous, 'status', text('replaced')); set(previous, 'terminalAt', text(now));
...
export function consumeAutofillAuthority(document: Document, jobId: string, now: string): Document | null {
  const validated = validateApplicationAuthorityDocument(document), record = activeApplicationAuthority(validated);
  if (record === null || string(get(record, 'mode')) !== 'autofill_to_review'
    || !(get(record, 'jobBindings') as Value[]).some(item => string(get(object(item, 'job binding'), 'jobId')) === jobId)) return null;
  set(record, 'status', text('consumed')); set(record, 'terminalAt', text(now));
  set(record, 'revision', integer(int(get(record, 'revision'))! + 1n)); set(validated, 'activeAuthorityId', null);
  const metadata = object(get(validated, 'metadata'), 'application authority metadata');
  set(metadata, 'revision', integer(int(get(metadata, 'revision'))! + 1n)); set(metadata, 'updatedAt', text(now));
