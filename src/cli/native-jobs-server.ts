...
const server = createServer(async (request, response) => {
  ...
  try {
    if (request.headers.host !== new URL(origin).host) return fail(403, "host_rejected", "request host is not the workspace");
    const actual = Buffer.from(request.headers.authorization ?? ""), expected = Buffer.from(`Bearer ${token}`);
    if (actual.length !== expected.length || !timingSafeEqual(actual, expected)) return fail(401, "token_rejected", "workspace token is missing or invalid");
    if (!["GET", "POST", "PATCH"].includes(request.method!)) return fail(405, "method_rejected", "method not allowed");
    const path = request.url ?? "";
    if (!path.startsWith("/api/") || path.includes("?") || path.includes("#")) return fail(404, "not_found", "route not found");
    try { decodeURIComponent(path); } catch { return fail(400, "request_error", "request path is invalid"); }
    let body = "";
    if (request.method !== "GET") {
      if (request.headers.origin !== origin) return fail(403, "origin_rejected", "request origin is not the workspace");
      ...
      catch { return fail(400, "request_error", "body must be UTF-8 JSON"); }
    }
    send(await jobsHttp(service, repository, request.method!, path, body));
  } catch { fail(503, "store_unavailable", "native fixture service is unavailable"); }
});
server.requestTimeout = 30_000;
server.headersTimeout = 30_000;
server.listen(0, "127.0.0.1", () => {
  const address = server.address();
  if (!address || typeof address === "string") throw new Error("native service did not bind");
