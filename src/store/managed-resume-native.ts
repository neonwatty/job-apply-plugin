...
import type { StatTimeMode } from "../contracts/stat-time.js";
import { managedResumePath } from "./managed-resume-path.js";
import { privateFileDigest } from "./private-file-digest.js";
import type { DigestIO } from "./private-file-digest.js";
import type { ManagedObservationIO } from "./managed-resume-observation.js";

export interface ManagedResumeNativeOptions {
  pathProfile: PythonPathProfile;
  statTimeMode: StatTimeMode;
...
function nativePath(path: string): Buffer {
  ...
  if (bytes.includes(0)) throw new NativePathValueError();
  return bytes;
}

function isOSError(error: unknown): error is NodeJS.ErrnoException {
...
export function createManagedResumeNativeIO(
  root: string,
  options: ManagedResumeNativeOptions,
): ManagedObservationIO {
  ...
  async function isSymlink(path: string): Promise<boolean> {
    try {
      ...
      if (error instanceof NativePathValueError || error instanceof FilesystemEncodeError) return false;
      if (isOSError(error) && (pathProfile === "3.14"
        || error.code === "ENOENT" || error.code === "ENOTDIR"
        || error.code === "EBADF" || error.code === "ELOOP")) return false;
      throw error;
    }
  }

  const digestIO: DigestIO = {
