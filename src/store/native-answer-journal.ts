...
export function validateIdleCoordinator(value: Document): Document {
...
export function validateAnswerJournal(document: Document): Document {
  if (document.size !== 2 || int(get(document, 'schemaVersion')) !== 1n || !document.has(text('operation'))) throw new JobsError('invalid coordinator journal');
  ...
  if (operation.size !== fields.length || keys(operation).some(key => !fields.includes(key)) || string(get(operation, 'kind')) !== 'answer_merge' || get(operation, 'resultClaim') !== null) throw new JobsError('unsupported coordinator operation');
  for (const field of ['operationId', 'at', 'winnerKey', 'sourceKey']) if (!string(get(operation, field))) throw new JobsError('coordinator answer merge operation is invalid');
  if (string(get(operation, 'winnerKey')) === string(get(operation, 'sourceKey'))) throw new JobsError('coordinator answer merge identity is invalid');
  for (const field of ['expectedWinnerRevision', 'expectedSourceRevision']) {
    const revision = int(get(operation, field));
    if (revision === null || revision < 1n) throw new JobsError('coordinator answer merge revision is invalid');
  }
  const sessions = get(operation, 'sessions');
  if (!Array.isArray(sessions)) throw new JobsError('coordinator answer merge sessions are invalid');
  const identities = new Set<string>();
  for (const value of sessions) {
    const session = validateAnswerSession(value), id = string(get(session, 'applicationId'))!;
    if (identities.has(id)) throw new JobsError('coordinator answer merge sessions are duplicated');
    identities.add(id);
  }
  return document;
}
export class NativeAnswerJournal {
  ...
  async recover(journal: Document, answers: Document, sessions: Document[]): Promise<void> {
    ...
    const targets = get(operation, 'sessions') as Document[];
    for (const session of targets) if (!known.has(string(get(session, 'applicationId')))) throw new JobsError('coordinator merge references a missing session');
    ...
    for (const session of targets) await this.write(`sessions/${string(get(session, 'applicationId'))!}`, session);
  ...
  async commit(operation: Document, answers: Document, sessions: Document[]): Promise<void> {
    const journal = object(fromJSON({ schemaVersion: 1, operation: null }), 'journal');
    journal.set(text('operation'), operation);
    validateAnswerJournal(journal);
    const known = new Set(sessions.map(session => string(get(session, 'applicationId'))));
    for (const session of get(operation, 'sessions') as Document[]) if (!known.has(string(get(session, 'applicationId')))) throw new JobsError('coordinator merge references a missing session');
...
function applyForValidation(answers: Document, operation: Document): Document {
