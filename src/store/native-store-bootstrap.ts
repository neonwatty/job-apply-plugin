...
function child(root: string, name: string): string { return root === '/' || root === '//' ? root + name : `${root}/${name}`; }
export function nativeStorePaths(root: string, legacyProfile: string): NativeStorePathEnvelope {
  root = pythonPath(root); legacyProfile = pythonPath(legacyProfile);
  return {
...
export class NativeStoreBootstrap {
  ...
  private readonly locked: <T>(operation: () => Promise<T>) => Promise<T>;
  constructor(root: string, legacyProfile: string = join(homedir(), '.claude-job-profile.json'), options: BootstrapOptions = {}) {
    this.root = pythonPath(root); this.legacyProfile = pythonPath(legacyProfile);
  ...
  private async preflight(): Promise<{ root: Identity | null; legacy: Document | null }> {
    ...
    if (rootIdentity !== null) {
      ...
      if (journalBytes !== null) {
        ...
        if (operation === null) validateAnswerJournal(journal);
        else {
          const kind = string(get(object(operation, 'coordinator operation'), 'kind'));
          if (kind === 'answer_resolution') validateResolutionJournal(journal);
          else if (claimOperationKinds.has(kind!)) { validateClaimJournal(journal); pendingClaim = true; }
          else validateAnswerJournal(journal);
        }
      }
      if (history !== null && !pendingClaim) await new NativeClaimHistory(this.root).read();
  ...
  private async readDocument(name: string): Promise<Document> {
  ...
  private async recoverCoordinator(): Promise<void> {
    ...
    if (!coordinatorExists && !journalExists) return;
    if (!coordinatorExists) await this.writeDocument('coordinator', object(fromJSON({ schemaVersion: 1, claim: null }), 'coordinator'));
    if (!journalExists) await this.writeDocument('coordinator-journal', object(fromJSON({ schemaVersion: 1, operation: null }), 'journal'));
    ...
    if (operation === null) { await history.read(); return; }
    const kind = string(get(object(operation, 'coordinator operation'), 'kind'));
    ...
    await history.read();
    if (get(coordinator, 'claim') !== null) throw new JobsError('answer recovery requires an idle coordinator');
  ...
  private async recoverResumeFiles(): Promise<void> {
    ...
    if (journalBytes !== null) {
      ...
      await new NativeResumeFiles(this.root).recover(operation === null ? null : object(operation, 'resume recovery operation'), resumes,
        ...
        () => this.writeDocument('resume-operation', object(fromJSON({ schemaVersion: 1, operation: null }), 'journal')));
      return;
    }
    const cached = new Map<string, Buffer>();
    for (const name of names) cached.set(name, (await readPrivateFile(join(directory, name), 'resume recovery file', false))!);
    for (const name of names) if (name.startsWith('.') && name.endsWith('.tmp')) await unlink(join(directory, name));
    for (const name of names) if (name.startsWith('.browser-upload.') && !name.endsWith('.tmp')) {
      if ((await lstat(join(directory, name))).mtimeMs <= Date.now() - 300_000) await unlink(join(directory, name));
    }
    ...
    for (const [, value] of records.entries()) {
      ...
      if (string(get(record, 'storageKind')) !== 'managed') continue;
      ...
      if (canonical === undefined || digest(canonical) !== expected) {
        ...
        if (recovery !== undefined) {
          if (canonical !== undefined) await unlink(join(directory, managed));
          await rename(join(directory, recovery), join(directory, managed));
          canonical = cached.get(recovery);
        }
      }
      if (canonical !== undefined && digest(canonical) === expected) {
        for (const name of quarantines) await unlink(join(directory, name)).catch(error => {
          if ((error as NodeJS.ErrnoException).code !== 'ENOENT') throw error;
        });
      }
    }
    for (const name of names) if (name.startsWith('.') && name.endsWith('.quarantine')) {
      await unlink(join(directory, name)).catch(error => { if ((error as NodeJS.ErrnoException).code !== 'ENOENT') throw error; });
    }
    for (const name of names) if (!name.startsWith('.') && /\.(?:pdf|docx|txt)$/i.test(name) && !referenced.has(name)) {
      await unlink(join(directory, name)).catch(error => { if ((error as NodeJS.ErrnoException).code !== 'ENOENT') throw error; });
    }
    const handle = await open(directory, constants.O_RDONLY | constants.O_NOFOLLOW);
    try { await handle.sync(); } finally { await handle.close(); }
  }
  async initialize(): Promise<Value> { return this.locked(() => this.initializeLocked()); }
  private async initializeLocked(): Promise<Value> {
...
function expandUser(path: string, home: string): string {
  ...
  if (path === '~' || path === current) return home;
  if (path.startsWith('~/')) return child(home, path.slice(2));
  if (path.startsWith(`${current}/`)) return child(home, path.slice(current.length + 1));
  return path;
}
function pythonPath(path: string): string {
...
export function createNativeStoreBootstrap(root?: string, legacyProfile?: string,
  environment: NodeJS.ProcessEnv = process.env, home = homedir(), options: BootstrapOptions = {}): NativeStoreBootstrap {
  ...
  return new NativeStoreBootstrap(expandUser(selectedRoot, home),
    expandUser(legacyProfile || join(home, '.claude-job-profile.json'), home), options);
}
