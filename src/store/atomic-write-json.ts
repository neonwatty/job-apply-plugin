...
import type { AtomicWriteIO } from "./private-filesystem.js";

function withContext(error: unknown, previous: unknown): unknown {
...
export async function atomicWriteJson(
  ...
  try {
    ...
    try {
      for (const chunk of iterPersistedJson(payload, options)) await temporary.write(chunk);
      ...
      try { await temporary.close(); } catch (error) { throw withContext(error, writeFailure); }
    }
    ...
    if (temporaryPath !== undefined) {
      try {
        ...
        if (!isFilesystemError(error) || error.code !== "ENOENT") throw withContext(error, failure);
      }
    }
  }
}
