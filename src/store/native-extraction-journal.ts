...
export type ExtractionUpdates = { profile?: Document; proposals?: Document; requests?: Document; resumes?: Document; facts?: Document };
const destinations = { profile: 'profile', proposals: 'resume-extractions', requests: 'resume-extraction-requests', resumes: 'resumes', facts: 'resume-facts' } as const;
...
export function closeRequestsForResumes(requestDocument: Document, resumeDocument: Document): Document | null {
  ...
  for (const [, value] of object(get(requests, 'requests'), 'requests.requests').entries()) {
    ...
    if (get(request, 'scope') !== null) set(request, 'factRevision', null);
    changed = true;
  }
