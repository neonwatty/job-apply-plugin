...
export interface JsonlHandle {
  stat(): Promise<{ size: bigint }>;
  write(bytes: Buffer): Promise<number>;
  read(length: bigint): Promise<Buffer>;
  truncate(size: bigint): Promise<void>;
  sync(): Promise<void>;
  close(): Promise<void>;
}
export interface JsonlHistoryIO {
  open(path: string, flags: number, mode?: number): Promise<JsonlHandle>;
...
export function createNativeJsonlHistoryIO(profile: PythonPathProfile): JsonlHistoryIO {
  validatePathProfile(profile);
  if (process.platform === "win32") throw new Error("Native Windows JSONL history IO is not supported by this POSIX adapter");
  return {
    async exists(path) {
      try {
