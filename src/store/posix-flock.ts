...
export interface PosixFlockProvider {
...
export function loadPosixFlockProvider(absoluteAddonPath: string): PosixFlockProvider {
  if (process.platform === "win32") throw new Error("POSIX flock is unavailable on native Windows");
  if (!isAbsolute(absoluteAddonPath)) throw new TypeError("Native flock artifact path must be absolute");
  ...
  return {
    tryLock(descriptor) {
      try {
        const result: unknown = Reflect.apply(tryLock, native, [descriptor]);
        if (typeof result !== "boolean") throw new TypeError("Native flock acquisition returned an invalid result");
        return result;
      } catch (error) { throw pythonFilesystemError(error); }
    },
    unlock(descriptor) {
      try { Reflect.apply(unlock, native, [descriptor]); }
      catch (error) { throw pythonFilesystemError(error); }
    },
  };
}
