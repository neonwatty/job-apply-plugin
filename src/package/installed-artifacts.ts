...
import { artifactBytes, artifactIsSymlink, artifactOSError, artifactRoot,
  ArtifactVerificationError, checkedArtifactPath, regularArtifactFile } from "./artifact-paths.js";
...
async function walkArtifacts(
  root: string, relative: string, profile: PythonPathProfile, paths: Set<string>,
): Promise<void> {
  ...
  try {
    ...
    if (artifactOSError(error)) return;
    throw error;
  }
...
export async function criticalPaths(root: string, profile: PythonPathProfile): Promise<string[]> {
  root = await artifactRoot(root, profile);
  const paths = new Set<string>(FIXED_CRITICAL_FILES);
  for (const relative of FIXED_CRITICAL_FILES) await regularArtifactFile(root, relative);
...
export async function assertCriticalBytes(
  installed: string, source: string, options: { label: string; profile: PythonPathProfile },
): Promise<void> {
