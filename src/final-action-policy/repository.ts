...
import type { AtomicWriteIO } from '../store/private-filesystem.js';
...
import type { Application, Attempt, OpaqueReference, ReferenceKind } from './model.js';

export interface PolicyOptions {
  provider: PosixFlockProvider;
  clock?: () => Date;
  newReference?: <K extends ReferenceKind>(kind: K) => OpaqueReference<K>;
  atomicIO?: AtomicWriteIO;
}
export const expandRoot = (root: string): string => root === '~' ? homedir() : root.startsWith('~/') ? join(homedir(), root.slice(2)) : root;
...
export async function readDocument(path: string, label: string): Promise<Record<string, unknown>> {
  let value: unknown;
  try { value = parsePolicyJson(decodePolicyBytes(await readFile(path))); }
  catch { throw new PolicyError(`${label} is unavailable or invalid`); }
...
export class PolicyRepository {
  ...
  protected readonly newReference: <K extends ReferenceKind>(kind: K) => OpaqueReference<K>;
  protected readonly io: AtomicWriteIO;
  private readonly provider: PosixFlockProvider;
  constructor(root: string, options: PolicyOptions) {
  ...
  protected locked<T>(operation: () => Promise<T>): Promise<T> {
    return withExclusiveFileLock(this.lockPath, operation, { provider: this.provider, pathProfile: serialization.pathProfile });
  }
  protected privateDirectory(path: string): Promise<void> { return ensurePrivateDirectory(path, this.io); }
  protected writeDocument(path: string, value: unknown): Promise<void> {
  ...
  protected campaignApplicationsDir(campaignId: string): string {
    reference(campaignId, 'campaign', 'campaignId');
    return join(this.applicationsDir, campaignId.split(':')[1]!);
  }
  protected applicationPath(applicationRef: string, campaignId: string): string {
  ...
  protected async loadApplication(applicationRef: string, campaignId: string): Promise<Application> {
  ...
  protected async campaignApplications(campaignId: string): Promise<Application[]> {
    const applications = [];
  ...
  protected newAttempt(number: number, now: Date, campaignExpiry: number): Attempt {
  ...
  protected async applicationFiles(campaignId: string): Promise<string[]> {
...
export function parseApplication(value: unknown): Application {
  ...
  const attempts = item.attempts.map((value, index) => {
    ...
    if (attempt.claimId !== null) reference(attempt.claimId, 'claim', 'claimId');
    ...
    if (attempt.outcomeAt !== null) parseTime(attempt.outcomeAt);
    if (attempt.confirmationRevision !== null) fingerprint(attempt.confirmationRevision, 'confirmationRevision');
    ...
    if (attempt.outcome === 'confirmed_submitted') check(attempt.confirmationRevision !== null, 'confirmed attempt has no confirmation');
    else check(attempt.confirmationRevision === null, 'attempt confirmation is invalid');
    return attempt;
  });
  if (attempts.length === 2) check(attempts[0]!.outcome === 'uncertain', 'retry does not follow uncertainty');
  ...
  check(stateValid[item.status as keyof typeof stateValid], 'application attempt state is inconsistent');
  parseTime(item.createdAt);
  parseTime(item.updatedAt);
  return structuredClone(item) as unknown as Application;
}
