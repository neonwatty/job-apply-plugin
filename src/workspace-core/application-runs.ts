...
const doc = (value: unknown): Document => object(fromJSON(value), 'application run value');

function jobIds(value: Value): string[] {
  if (!Array.isArray(value)) throw new JobsError('application run jobIds must be a list');
  const result = value.map(item => safeId(string(item)));
  if (new Set(result).size !== result.length) throw new JobsError('application run queue contains duplicate jobs');
  return result;
}

function inputJobIds(input: Document): string[] {
  if (input.size !== 1 || keys(input)[0] !== 'jobIds') throw new JobsError('application run input must contain only jobIds');
  return jobIds(get(input, 'jobIds'));
}

function requireJobs(jobs: Document, ids: string[]): void {
  const records = object(get(jobs, 'jobs'), 'jobs');
  for (const id of ids) {
    const value = get(records, id);
    if (value === null || get(object(value, 'job'), 'deletedAt') !== null) throw new JobsError(`application run job is unavailable: ${id}`);
  }
}
...
export class ApplicationRunsService {
  ...
  start(resumeId: string, expectedResume: bigint, expectedFacts: bigint, confirmed: boolean, input: Document): Promise<Value> {
    safeId(resumeId);
    if (!confirmed) throw new JobsError('application run inputs require owner confirmation in chat');
    ...
    return this.repository.claimTransaction(async tx => {
      ...
      if (applicationRuns === null) { applicationRuns = doc({ activeRunId:null, runs:{} }); set(metadata, 'applicationRuns', applicationRuns); }
      set(object(get(applicationRuns, 'runs'), 'application runs'), runId, run);
      set(applicationRuns, 'activeRunId', text(runId)); set(metadata, 'updatedAt', text(now));
      await tx.saveJobs(tx.jobs);
      return run;
    });
  }

  update(runId: string, expectedRevision: bigint, input: Document): Promise<Value> {
    safeId(runId); const ids = inputJobIds(input);
    return this.repository.claimTransaction(async tx => {
      const run = activeApplicationRun(tx.jobs);
      if (run === null || string(get(run, 'runId')) !== runId) throw new JobsError('application run is not active');
      if (int(get(run, 'revision')) !== expectedRevision) throw new JobsError('application run revision conflict');
      requireJobs(tx.jobs, ids);
      const claim = get(tx.coordinator, 'claim');
      if (claim !== null) {
        const claimedId = string(get(object(claim, 'claim'), 'jobId'))!;
        if (!ids.includes(claimedId)) throw new JobsError('active claimed job cannot be removed from its application run');
      }
      if (currentRunJobIds(run).join('\0') === ids.join('\0')) return run;
      const now = this.now(), nextRevision = expectedRevision + 1n, next = copy(run);
      const versions = [...get(run, 'queueVersions') as Value[]];
      ...
      set(next, 'queueVersions', versions); set(next, 'revision', integer(nextRevision)); set(next, 'updatedAt', text(now));
      const metadata = object(get(tx.jobs, 'metadata'), 'jobs metadata');
      const runs = validateApplicationRuns(metadata)!;
      set(object(get(runs, 'runs'), 'application runs'), runId, next); set(metadata, 'updatedAt', text(now));
      await tx.saveJobs(tx.jobs); return next;
    });
  }

  complete(runId: string, expectedRevision: bigint): Promise<Value> {
    safeId(runId);
    return this.repository.claimTransaction(async tx => {
      const run = activeApplicationRun(tx.jobs);
      if (run === null || string(get(run, 'runId')) !== runId) throw new JobsError('application run is not active');
      if (int(get(run, 'revision')) !== expectedRevision) throw new JobsError('application run revision conflict');
      if (get(tx.coordinator, 'claim') !== null) throw new JobsError('application run cannot complete while a job is claimed');
      const now = this.now(), next = copy(run), metadata = object(get(tx.jobs, 'metadata'), 'jobs metadata');
      set(next, 'status', text('completed')); set(next, 'revision', integer(expectedRevision + 1n));
      set(next, 'updatedAt', text(now)); set(next, 'completedAt', text(now));
      const runs = validateApplicationRuns(metadata)!;
      set(object(get(runs, 'runs'), 'application runs'), runId, next); set(runs, 'activeRunId', null); set(metadata, 'updatedAt', text(now));
      await tx.saveJobs(tx.jobs); return next;
    });
  }
}
