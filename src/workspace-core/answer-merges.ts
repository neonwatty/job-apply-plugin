...
export interface AnswerMergeTransaction {
  document: Document;
  sessions: Document[];
  history: Document[];
...
export class AnswerMergeService {
  constructor(private readonly repository: AnswerRepository, private readonly now = () => new Date().toISOString().replace(/\.\d{3}Z$/, 'Z')) {}
  approve(incoming: Value, ownerConfirmed: boolean): Promise<Value> {
    ...
    if (packet.size !== fields.length || keys(packet).some(key => !fields.includes(key)) || ownerConfirmed !== true || string(get(packet, 'previewToken')) === null) throw new JobsError('answer cleanup requires explicit owner approval');
  ...
  async merge(winnerKey: string, sourceKey: string, winnerRevision: bigint, sourceRevision: bigint, approval: Document | null = null): Promise<Value> {
    if (typeof winnerKey !== 'string' || !winnerKey || typeof sourceKey !== 'string' || !sourceKey) throw new JobsError('answer merge keys must be non-empty strings');
    if (winnerKey === sourceKey) throw new JobsError('answer merge requires distinct records');
    if (typeof winnerRevision !== 'bigint' || winnerRevision < 1n || typeof sourceRevision !== 'bigint' || sourceRevision < 1n) throw new JobsError('answer merge revisions must be positive integers');
    if (!this.repository.answerMergeTransaction) throw new JobsError('answer merge requires coordinator support');
    return this.repository.answerMergeTransaction(async transaction => {
      ...
      if (approval) {
        ...
        if (actual.length !== supplied.length || !timingSafeEqual(actual, supplied)) throw new JobsError('answer cleanup preview is stale');
        const selected = (get(preview, 'proposals') as Document[]).some(proposal => ['winnerKey', 'duplicateKey', 'winnerRevision', 'duplicateRevision'].every(key => same(get(proposal, key), get(approval, key))));
        if (!selected) throw new JobsError('answer cleanup selection is not in the preview');
      }
      const redirects = get(document, 'redirects');
      if (redirects !== null && (has(object(redirects, 'redirects'), winnerKey) || has(object(redirects, 'redirects'), sourceKey))) throw new JobsError('answer merge records must be canonical active records');
      const answers = object(get(document, 'answers'), 'answers');
      if (!has(answers, winnerKey) || !has(answers, sourceKey)) throw new JobsError('answer merge record does not exist');
      const at = this.now();
