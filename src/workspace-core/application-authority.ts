...
import { activeApplicationAuthority, applicationAuthorityInterrupt, applicationAuthorityModes,
  applicationAuthorityOperations, applicationAuthorityProjection, applicationOrigin, replaceAuthority,
  validateApplicationAuthority, validateApplicationAuthorityDocument, validateBoundSensitiveAnswer }
  from '../contracts/workspace/application-authority.js';
...
export interface ApplicationAuthorityTransaction {
  authority: Document; jobs: Document; coordinator: Document; profile: Document; resumes: Document;
  facts: Document; requests: Document; answers: Document; files: NativeResumeFiles;
  saveAuthority(document: Document): Promise<void>;
}
export interface ApplicationAuthorityRepository {
  applicationAuthorityTransaction<T>(operation: (transaction: ApplicationAuthorityTransaction) => Promise<T>): Promise<T>;
}

const doc = (value: unknown): Document => object(fromJSON(value), 'application authority value');
const positive = (value: Value, label: string, zero = false): bigint => {
...
function strings(value: Value, label: string): string[] {
  if (!Array.isArray(value)) throw new JobsError(`${label} must be a list`);
  const result = value.map(item => string(item));
  if (result.some(item => !item) || new Set(result).size !== result.length) throw new JobsError(`${label} is invalid`);
  return result as string[];
}
function exact(record: Document, fields: string[], label: string): void {
  if (record.size !== fields.length || keys(record).some(key => !fields.includes(key))) throw new JobsError(`${label} contains unsupported fields`);
}
function currentAnswer(answers: Document, ref: string): Document | null {
...
export class ApplicationAuthorityService {
  ...
  progress(): Promise<Document> {
    return this.repository.applicationAuthorityTransaction(async tx => {
      ...
      for (const binding of get(record, 'jobBindings') as Value[]) {
        ...
        const key = fields.get(status) ?? 'unavailable'; set(counts, key, integer(int(get(counts, key))! + 1n));
        const item = doc({ jobId:id, status, revision:null });
        if (raw !== null) set(item, 'revision', get(object(raw, 'job'), 'revision'));
        jobs.push(item); if (next === null && status === 'ready') next = item;
      }
      set(result, 'jobs', jobs); if (next !== null) set(result, 'nextJob', next); return result;
    });
  }

  set(value: Value, expectedRevision: bigint): Promise<Document> {
    ...
    if (!applicationAuthorityModes.has(mode ?? '')) throw new JobsError('application authority mode is invalid');
    ...
    return this.repository.applicationAuthorityTransaction(async tx => {
      ...
      const resume = object(resumeValue, 'resume'), bindings: Value[] = [];
      ...
      for (const id of ids) {
        ...
        set(binding, 'resumeRevision', get(resume, 'revision')); set(binding, 'factRevision', get(selection, 'factRevision'));
        bindings.push(binding);
      }
      const sensitive: Value[] = [];
      for (const ref of sensitiveRefs) {
        const answer = currentAnswer(tx.answers, ref);
        if (answer === null || !sensitiveAnswer(answer)) throw new JobsError('sensitive answer authority requires an accepted confirmed answer');
        const binding = doc({ answerRef: ref, answerRevision: null });
        set(binding, 'answerRevision', integer(answerRevision(answer))); sensitive.push(binding);
      }
  ...
  revoke(expectedRevision: bigint): Promise<Document> {
    return this.repository.applicationAuthorityTransaction(async tx => {
      ...
      if (record === null) throw new JobsError('application authority is already Guided');
      const now = this.now(); set(record, 'status', text('revoked')); set(record, 'terminalAt', text(now));
      set(record, 'revision', integer(int(get(record, 'revision'))! + 1n)); set(authority, 'activeAuthorityId', null);
      set(metadata, 'revision', integer(expectedRevision + 1n)); set(metadata, 'updatedAt', text(now));
      await tx.saveAuthority(authority); return applicationAuthorityProjection(authority, now);
    });
  }

  control(action: 'pause' | 'resume' | 'stop', expectedRevision: bigint): Promise<Document> {
    return this.repository.applicationAuthorityTransaction(async tx => {
      const authority = validateApplicationAuthorityDocument(tx.authority), metadata = object(get(authority, 'metadata'), 'authority metadata');
      if (int(get(metadata, 'revision')) !== expectedRevision) throw new JobsError('application authority revision conflict');
      const record = activeApplicationAuthority(authority);
      if (record === null || string(get(record, 'mode')) !== 'campaign_to_review') throw new JobsError('campaign authority is not active');
      const current = string(get(record, 'status'))!, now = this.now();
      if (action === 'pause' && current !== 'active' || action === 'resume' && current !== 'paused'
        || action === 'stop' && !['active','paused'].includes(current)) throw new JobsError(`campaign cannot ${action} from its current state`);
      ...
      if (action === 'stop') { set(record, 'terminalAt', text(now)); set(authority, 'activeAuthorityId', null); }
      set(metadata, 'revision', integer(expectedRevision + 1n)); set(metadata, 'updatedAt', text(now));
      await tx.saveAuthority(authority); return applicationAuthorityProjection(authority, now);
    });
  }

  evaluate(value: Value): Promise<Document> {
    const input = object(value, 'application authority evaluation');
    exact(input, ['jobId', 'claimToken', 'destinationUrl', 'operations', 'answerRefs', 'sensitiveAnswerRefs', 'interrupts'],
      'application authority evaluation');
    const interrupt = applicationAuthorityInterrupt(get(input, 'interrupts'));
    if (interrupt !== null) return Promise.resolve(doc({ authorized: false, mode: 'guided', reasonCode: interrupt, interrupt: true }));
    ...
    if (operations.some(operation => !applicationAuthorityOperations.has(operation))) throw new JobsError('application authority operations are invalid');
    ...
    return this.repository.applicationAuthorityTransaction(async tx => {
      ...
      if (record === null) return doc({ authorized:false, mode:'guided', reasonCode:'granular_confirmation_required', interrupt:true });
      if (string(get(record, 'status')) !== 'active') return doc({ authorized:false, mode:'guided', reasonCode:'authority_not_active', interrupt:true });
      if (claimTime(now) >= claimTime(string(get(record, 'expiresAt'))!)) return doc({ authorized:false, mode:'guided', reasonCode:'authority_expired', interrupt:true });
      const run = activeApplicationRun(tx.jobs);
      ...
      if (!binding) return doc({ authorized:false, mode:'guided', reasonCode:'job_out_of_scope', interrupt:true });
      ...
      try { requireClaim(tx.coordinator, tx.jobs, id, token, now); }
      catch { return doc({ authorized:false, mode:'guided', reasonCode:'claim_missing_or_expired', interrupt:true }); }
      ...
      for (const ref of answerRefs) {
        const answer = currentAnswer(tx.answers, ref);
        if (answer === null || sensitiveAnswer(answer)) return doc({ authorized:false, mode:'guided', reasonCode:'answer_not_confirmed_non_sensitive', interrupt:true });
      }
      const bound = new Map((get(record, 'sensitiveAnswerBindings') as Value[]).map(item => {
        const binding = object(item, 'sensitive answer binding'); return [string(get(binding, 'answerRef'))!, binding];
      }));
      for (const ref of sensitiveRefs) {
      ...
      set(result, 'revision', get(object(get(authority, 'metadata'), 'authority metadata'), 'revision')); return result;
    });
  }
}
