...
export async function profileHttp(repository: NativeJobsRepository, method: string, path: string, body: string): Promise<{ status: number; body: string } | null> {
  ...
  if (method === 'GET' && path === '/api/profile') return response(await profile.inspect());
  if (method === 'GET' && path === '/api/fact-groups') return response(set(emptyObject(), 'groups', await groups.list()));
  ...
  if (method === 'GET' && match && !match[2]) {
    ...
    if (group === null) throw new JobsError('fact group does not exist');
  ...
  if (method === 'PATCH' && path === '/api/profile') {
    ...
    const lists = (key: string): string[] => {
      ...
      if (!Array.isArray(value) || value.some(item => string(item) === null)) throw new JobsError('body requires valid path lists');
      return value.map(item => string(item)!);
    };
    if (keys(payload).some(key => !['patch', 'expectedRevision', 'atomicPaths', 'deletedPaths'].includes(key)) || !(patch instanceof PythonObject) || !patch.size) throw new JobsError('body requires a non-empty patch object, expectedRevision, and valid path lists');
    const atomic = lists('atomicPaths'), deleted = lists('deletedPaths');
    if (atomic.some(path => named.has(topKey(path))) || new Set(atomic).size !== atomic.length || new Set(deleted).size !== deleted.length || deleted.some(path => !atomic.includes(path))) throw new JobsError('atomic paths must uniquely identify Additional facts and include every deletion');
  ...
  if (method === 'POST' && path === '/api/fact-groups') {
    ...
    if (payload.size !== 1 || keys(payload)[0] !== 'group' || !(get(payload, 'group') instanceof PythonObject)) throw new JobsError('body must contain only a fact group object');
  ...
  if (match && (method === 'PATCH' && !match[2] || method === 'POST' && match[2])) {
    ...
    if (deleting ? payload.size !== 1 || keys(payload)[0] !== 'expectedRevision'
      : payload.size !== 2 || keys(payload).some(key => !['patch', 'expectedRevision'].includes(key)) || !(get(payload, 'patch') instanceof PythonObject)) throw new JobsError(deleting ? 'delete body requires expectedRevision' : 'body requires patch and expectedRevision');
...
function revision(value: Value): bigint {
  const result = int(value);
  if (result === null || result < 1n) throw new JobsError('expectedRevision must be a positive integer');
  return result;
}
