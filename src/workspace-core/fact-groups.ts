...
export interface GroupsRepository {
  groupsTransaction<T>(operation: (document: Document, save: (document: Document) => Promise<void>) => Promise<T>): Promise<T>;
}
export class FactGroupsService {
  constructor(readonly repository: GroupsRepository,
    private readonly now = () => new Date().toISOString().replace(/\.\d{3}Z$/, 'Z'),
    private readonly uuid = () => randomUUID().replaceAll('-', '')) {}
  list(): Promise<Document[]> {
    return this.repository.groupsTransaction(async document => object(get(validateGroups(document), 'groups'), 'fact groups.groups').entries()
      .map(([, value]) => object(value, 'fact group record')).sort((a,b) =>
        Number(int(get(a, 'order'))! - int(get(b, 'order'))!)
  ...
  async get(groupId: string): Promise<Value> {
    id(groupId);
  ...
  async create(value: Value): Promise<Document> {
    const input = object(value, 'fact group');
    if (!has(input, 'label') || !has(input, 'paths') || keys(input).some(key => !['label', 'paths', 'order'].includes(key))) throw new JobsError('fact group requires label and paths');
    ...
    return this.repository.groupsTransaction(async (document, save) => {
      ...
      const record = fromJSON({ id: groupId, label: name, revision: 1, createdAt: now, updatedAt: now }) as Document;
  ...
  async update(groupId: string, value: Value, revision: bigint): Promise<Document> {
    id(groupId);
    const input = object(value, 'fact group patch');
    if (!input.size || keys(input).some(key => !['label', 'paths', 'order'].includes(key))) throw new JobsError('fact group patch must contain label, paths, or order');
    return this.change(groupId, revision, async (document, groups, current, save) => {
      ...
      if (has(input, 'paths')) set(next, 'paths', paths(get(input, 'paths')));
      if (has(input, 'order')) set(next, 'order', integer(order(get(input, 'order'))));
      if (['label', 'paths', 'order'].every(key => same(get(next, key), get(current, key)))) return current;
      const now = this.now();
  ...
  async delete(groupId: string, revision: bigint): Promise<Document> {
    return this.change(groupId, revision, async (document, groups, _current, save) => {
      ...
      await save(document);
      return fromJSON({ deleted: true, id: groupId }) as Document;
    });
  }
  private change<T>(groupId: string, revision: bigint, operation: (document: Document, groups: Document, current: Document, save: (document: Document) => Promise<void>) => Promise<T>): Promise<T> {
    id(groupId);
    if (revision < 1n) throw new JobsError('fact group expected revision must be a positive integer');
    return this.repository.groupsTransaction(async (document, save) => {
      const groups = object(get(validateGroups(document), 'groups'), 'fact groups.groups'), value = get(groups, groupId);
      if (value === null) throw new JobsError('fact group does not exist');
      const current = object(value, 'fact group record');
      if (int(get(current, 'revision')) !== revision) throw new JobsError('fact group revision conflict');
      return operation(document, groups, current, save);
    });
  }
  private unique(groups: Document, name: string, except?: string): void {
    if (groups.entries().some(([key, value]) => string(key) !== except && casefold(string(get(object(value, 'fact group record'), 'label'))!) === casefold(name))) throw new JobsError('active fact group label already exists');
  }
}
