...
const agentBlockers: Record<string, string> = {
...
const canonical = (value: Value): string => canonicalJson(value);

function stringArray(value: Value, error: string): string[] {
  if (!Array.isArray(value) || value.some(item => string(item) === null)) throw new JobsError(error);
...
function readinessBlockerType(code: string): string {
...
function pendingReference(applicationId: string, value: Value): string {
  return `pending_${hash(JSON.stringify({ applicationId, pendingField: JSON.parse(canonical(value)) })).slice(0, 32)}`;
}
function projectPending(applicationId: string, raw: Value, reference: () => string, legacy: boolean,
  ats?: string | null): Document {
  const field = object(raw, 'pending field');
  const allowed = legacy ? legacyPendingFields : modernFields;
  if (keys(field).some(key => !allowed.has(key))) throw new JobsError('pending field contains unsupported fields');
  const result = copy(field), question = string(get(result, 'question'));
  if (has(result, 'question')) result.delete(text('question'));
  if (question?.trim()) set(result, 'questionFingerprint', text(hash(casefold(question.trim().replace(/\s+/g, ' ')))));
  if (!has(result, 'scopeFingerprint') && ats) {
    set(result, 'scopeFingerprint', text(hash(canonical(fromJSON({ ats })))));
  }
  if (!has(result, 'reference')) set(result, 'reference', text(legacy ? pendingReference(applicationId, raw) : reference()));
  const opaque = string(get(result, 'reference'));
  if (!opaque || !/^pending_[A-Za-z0-9_-]{1,128}$/.test(opaque)) throw new JobsError('pending field reference is invalid');
  return result;
}

function projectSession(value: Value, expectedId: string, ats?: string | null, reference = () => `pending_${randomBytes(16).toString('hex')}`): Document {
  const source = object(value, 'session'), id = safeId(string(get(source, 'applicationId')));
  if (id !== expectedId) throw new JobsError('session application id does not match path');
  const pending = get(source, 'pendingFields');
  if (!Array.isArray(pending)) throw new JobsError('session pendingFields must be a list');
  ...
  if (ats !== undefined) set(result, 'ats', ats === null ? null : text(ats));
  set(result, 'pendingFields', pending.map(item => projectPending(id, item, reference, legacy,
    string(get(result, 'ats')))));
  return result;
}
...
export class SessionService {
  constructor(readonly repository: SessionRepository,
    ...
    private readonly reference = () => `pending_${randomBytes(16).toString('hex')}`) {}

  save(id: string, value: Value): Promise<Document> {
    id = safeId(id); const input = object(value, 'session input');
    if (keys(input).some(key => !inputFields.has(key))) throw new JobsError('session contains unsupported fields');
    if (has(input, 'applicationId') && string(get(input, 'applicationId')) !== id) throw new JobsError('session application id does not match path');
    const status = has(input, 'status') ? string(get(input, 'status')) : 'active';
    if (!statuses.has(status!)) throw new JobsError('session status is unsupported');
    ...
    return this.repository.sessionTransaction(async transaction => {
      ...
      if (!jobAllowsMutation(job)) throw new JobsError('canonical job sessions require a coordinator operation');
      ...
      for (const [key, item] of input.entries()) if (!['applicationId', 'company', 'role', 'url', 'answerKeys', 'pendingFields',
        'createdAt', 'updatedAt', 'readinessInput', 'blockers', 'browserHandoff'].includes(string(key)!)) result.set(key, item);
      if (existingDocument !== null) {
        set(result, 'createdAt', get(existingDocument, 'createdAt'));
        if (!has(input, 'ats') && has(existingDocument, 'ats')) set(result, 'ats', get(existingDocument, 'ats'));
      }
      ...
      const blockers: Document[] = projected.map(field => {
      ...
      if (has(input, 'readinessInput')) {
        ...
        if (attempt === null || attempt < 1n) throw new JobsError('readiness requires a current attempt revision');
        ...
        if (!Array.isArray(codes) || codes.some(code => string(code) === null)) throw new JobsError('session readiness is invalid');
        for (const code of codes.map(code => string(code)!)) blockers.push(object(fromJSON({ type: readinessBlockerType(code), code }), 'session blocker'));
        const fallback = string(get(record, 'fallbackCode'));
        if (fallback !== null) blockers.push(object(fromJSON({ type: 'browser_handoff', code: fallback }), 'session blocker'));
      ...
      if (!Array.isArray(supplied)) throw new JobsError('session blockers must be a list');
      for (const raw of supplied) {
        ...
        if (code === null || agentBlockers[code] !== type) throw new JobsError('session blocker is invalid');
      ...
      if (blockers.some(blocker => string(get(blocker, 'type')) === 'browser_handoff')
        && string(get(handoff, 'state')) !== 'required') throw new JobsError('browser handoff contradicts browser blockers');
      set(result, 'browserHandoff', handoff);
      await transaction.save(id, result); return copy(result);
    });
  }
